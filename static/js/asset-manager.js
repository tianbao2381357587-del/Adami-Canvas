...
function assetCategories(){
    return (activeAssetLibrary()?.categories || []).filter(cat => ['image','media','video','audio'].includes(String(cat.type || 'image').toLowerCase()));
}
...
function assetViewTitle(){
    ...
    return activeAssetCategory()?.name || '资产库';
}
...
function assetCountForLibrary(lib){
    return (lib?.categories || [])
        .filter(cat => ['image','media','video','audio'].includes(String(cat.type || 'image').toLowerCase()))
...
function assetClassificationEntriesForLibrary(lib=activeAssetLibrary()){
    const items = [];
    (lib?.categories || []).filter(cat => ['image','media','video','audio'].includes(String(cat.type || 'image').toLowerCase())).forEach(cat => items.push(...(cat.items || [])));
...
function assetCategoryIcon(cat){
    const type = String(cat?.type || 'image').toLowerCase();
    if(type === 'audio') return 'file-audio';
    if(type === 'video') return 'file-video';
    return 'image';
}
function assetCategoryTypeLabel(type){
    const value = String(type || 'image').toLowerCase();
    if(value === 'audio') return '音频';
    if(value === 'video') return '视频';
    return '图片';
}
function assetCategoryUploadAccept(cat){
    const type = String(cat?.type || 'image').toLowerCase();
    if(type === 'audio') return 'audio/*,.mp3,.wav,.m4a,.aac,.ogg,.flac';
    if(type === 'video') return 'video/*,.mp4,.webm,.mov,.m4v,.mkv';
    return 'image/*,.png,.jpg,.jpeg,.webp,.gif,.bmp,.avif,.tif,.tiff';
}
function fileMatchesAssetCategory(file, cat){
    const type = String(cat?.type || 'image').toLowerCase();
    const mime = String(file?.type || '').toLowerCase();
    const name = String(file?.name || '').toLowerCase();
    if(type === 'audio') return mime.startsWith('audio/') || /\.(mp3|wav|flac|ogg|m4a|aac)(\?|#|$)/i.test(name);
    if(type === 'video') return mime.startsWith('video/') || /\.(mp4|webm|mov|m4v|mkv)(\?|#|$)/i.test(name);
    return mime.startsWith('image/') || /\.(png|jpe?g|webp|gif|bmp|avif|tiff?)(\?|#|$)/i.test(name);
}
...
function assetMoveTargets(){
    ...
    const targets = [];
    assetLibraries().forEach(lib => {
        (lib.categories || []).filter(cat => ['image','media','video','audio'].includes(String(cat.type || 'image').toLowerCase())).forEach(cat => {
...
function renderLocalClipboardBar(){
    ...
    return `<div class="asset-clipboard-bar">
        <div class="asset-clipboard-info"><i data-lucide="clipboard"></i><span>${escapeHtml(modeLabel)}了 ${localClipboard.items.length} 个本地素材，目标：${escapeHtml(activeAssetLibrary()?.name || '资产库')} / ${escapeHtml(target?.name || '未选择分组')}</span></div>
        <div class="asset-tools">
            <button class="asset-btn primary" type="button" data-local-import-clipboard ${target ? '' : 'disabled'}><i data-lucide="clipboard-paste"></i><span>导入到资产库</span></button>
        ...
        <section class="asset-panel asset-content ${assetManageMode ? 'manage-on' : ''}">
            ...
            <div class="content-scroll">
                <div class="asset-grid">
                    ...
                    ${items.length ? '' : '<div class="empty-state">当前分组还没有素材，可以上传，或从智能画布输出保存到资产库。</div>'}
                </div>
            </div>
        </section>
        <aside class="asset-panel asset-detail">
...
function renderAssetTreeBranch(lib){
    ...
    const cats = (lib.categories || []).filter(cat => ['image','media','video','audio'].includes(String(cat.type || 'image').toLowerCase()));
    ...
    return `<div class="tree-branch ${isActiveLib ? 'expanded' : ''}">
        ...
        <div class="tree-children">
            ${cats.length ? cats.map(cat => `<button class="tree-row tree-child ${isActiveLib && cat.id === activeAssetCategoryId && assetTreeFocus === 'category' ? 'active' : ''}" type="button" data-asset-cat="${escapeAttr(cat.id)}" data-asset-cat-lib="${escapeAttr(lib.id)}">
                <span class="tree-elbow"></span>
                <span class="tree-row-icon"><i data-lucide="${assetCategoryIcon(cat)}"></i></span>
...
function renderAssetTreeInlineEdit(kind){
    ...
    const typeSelect = assetTreeEdit.kind === 'category-new'
        ? `<select id="assetTreeEditType" title="素材类型">
            ${['image','audio','video'].map(type => `<option value="${type}" ${String(assetTreeEdit.type || 'image') === type ? 'selected' : ''}>${assetCategoryTypeLabel(type)}</option>`).join('')}
        </select>`
        : '';
...
async function uploadFiles(files){
    const cat = activeAssetCategory();
    if(!cat) throw new Error('请先创建素材分组');
    const list = [...files].filter(file => fileMatchesAssetCategory(file, cat));
    if(!list.length){
        const type = String(cat.type || 'image').toLowerCase();
        if(type === 'audio') throw new Error('当前是音频素材分组，只能上传音频文件');
        if(type === 'video') throw new Error('当前是视频素材分组，只能上传视频文件');
        throw new Error('当前是图片素材分组，只能上传图片文件');
    }
...
async function handleClick(event){
    ...
    if(target.closest?.('[data-asset-upload]')){
        if(uploadInput) uploadInput.accept = assetCategoryUploadAccept(activeAssetCategory());
        uploadInput?.click();
        return;
    }
    ...
    if(target.closest?.('[data-asset-cat-new]')){
        ...
        assetTreeEdit = {kind:'category-new', value:'新分组', label:'分组名称', type:'image'};
        pendingTreeDelete = '';
        render(); return;
    }
...
async function saveAssetTreeEdit(){
    ...
    if(assetTreeEdit.kind === 'library-new'){
        ...
        const type = document.getElementById('assetTreeEditType')?.value || assetTreeEdit.type || 'image';
        data = await apiJson('/api/asset-library/categories', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({library_id:activeAssetLibraryId, name, type})});
        assetLibrary = data.library || assetLibrary;
...
function setLocalClipboard(mode, ids=null){
    ...
    pendingBatchDelete = '';
    render();
    setStatus(`复制了 ${items.length} 个共享素材，导入后会拷贝到素材分组（共享文件夹原文件保留）`);
}
async function pasteLocalClipboardToAssets(){
    if(!localClipboard?.items?.length) return;
    if(!activeAssetCategory()){
        setStatus('请先在资产库中创建或选择分组');
        return;
    }
    const clip = localClipboard;
    // 按所属共享文件夹分组，调用后端按路径导入（复制到资产库，无需走浏览器文件对象）
    const groups = new Map();
    ...
    selectedLocalId = '';
    render();
    setStatus(`已导入 ${imported} 个素材到资产库`);
}
...
document.addEventListener('change', event => {
    if(event.target?.id !== 'assetTreeEditType' || !assetTreeEdit || assetTreeEdit.kind !== 'category-new') return;
    const input = document.getElementById('assetTreeEditInput');
    const previousDefault = `${assetCategoryTypeLabel(assetTreeEdit.type || 'image')}素材`;
    const nextType = event.target.value || 'image';
    const nextDefault = `${assetCategoryTypeLabel(nextType)}素材`;
    assetTreeEdit.type = nextType;
    if(input && (!input.value.trim() || input.value === '新分组' || input.value === previousDefault)){
        input.value = nextDefault;
        assetTreeEdit.value = nextDefault;
    }
});
...
/* === Adami-Canvas compatibility shim === */
function assetCount(){
    try { return assetCountForLibrary(activeAssetLibrary()); } catch(e) { return 0; }
}

/* === Adami-Canvas custom overrides: hide smart classification / smart canvas / keep prompt library empty === */
renderLocalCaptionTools = function(imageCount){
    normalizeLocalCaptionSettings();
    const providers = localCaptionProviders();
    const models = localCaptionModels();
    const captionDisabled = !imageCount || !providers.length || !localCaptionModel || localCaptionBusy || localClassifyBusy;
...
canvasAssetCategories = function(){
    const count = (canvasAssetsData.items || []).filter(item => item.canvas_kind !== 'smart').length;
    const canvasCount = (canvasAssetsData.canvases || []).filter(item => item.kind !== 'smart').length;
    return [{id:'classic', name:'普通画布', count, canvas_count:canvasCount}];
};
activeCanvasAssetCategoryInfo = function(){
    return {id:'classic', name:'普通画布', count:0, canvas_count:0};
};
defaultCanvasAssetCategory = function(){ return 'classic'; };
canvasKindLabel = function(kind){ return '普通画布'; };
renderAssetTreeBranch = function(lib){
    const isActiveLib = lib.id === activeAssetLibraryId;
    const cats = (lib.categories || []).filter(cat => ['image','media','video','audio'].includes(String(cat.type || 'image').toLowerCase()));
    const showLibActions = isActiveLib && assetTreeFocus === 'library';
    return `<div class="tree-branch ${isActiveLib ? 'expanded' : ''}">
...
renderAssetManager = function(){
    ...
    root.innerHTML = `
        <aside class="asset-panel asset-nav">
            <div class="panel-head">
                <div class="panel-title"><strong>资产库</strong><span>按库和分组管理素材资源</span></div>
                <div class="panel-actions compact-actions">
                    ${assetTreeEdit?.placement === 'head' ? '' : '<button class="asset-icon-btn" type="button" data-asset-lib-new title="新建资产库"><i data-lucide="plus"></i></button>'}
                    ${renderHeadTreeInlineEdit(assetTreeEdit, 'assetTreeEditInput', 'data-asset-tree-edit-save', 'data-asset-tree-edit-cancel')}
                </div>
            ...
            <div class="nav-scroll">
                <div class="nav-tree">${libs.map(item => renderAssetTreeBranch(item)).join('')}</div>
            </div>
        </aside>
        <section class="asset-panel asset-content ${assetManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                <div class="content-heading">
                    <strong>${escapeHtml(cat?.name || '素材')}</strong>
                    <span>${assetViewSubtitle(items)}</span>
                </div>
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search"></i><input id="assetSearch" class="asset-search" type="search" value="${escapeAttr(assetQuery)}" placeholder="搜索名称、链接或类型"></label>
                    <button class="asset-btn primary" type="button" data-asset-upload ${cat ? '' : 'disabled'}><i data-lucide="upload"></i><span>上传</span></button>
                    <button class="asset-btn ${assetManageMode ? 'primary' : ''}" type="button" data-asset-manage ${assetCountForLibrary(activeAssetLibrary()) ? '' : 'disabled'}><i data-lucide="list-checks"></i><span>${assetManageMode ? '完成管理' : '批量管理'}</span></button>
                </div>
            </div>
            <div class="manage-tools">
                ...
                <div class="asset-tools">
                    ...
                    <button class="asset-btn" type="button" data-asset-copy-selected ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="copy"></i><span>复制</span></button>
                    <button class="asset-btn" type="button" data-asset-cut-selected ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="scissors"></i><span>剪切</span></button>
                    <button class="asset-btn" type="button" data-asset-download-selected ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="download"></i><span>下载</span></button>
                    <button class="asset-btn" type="button" data-asset-canvas-selected ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="clipboard-paste"></i><span>复制到画布</span></button>
                    <button class="asset-btn danger ${pendingBatchDelete === 'asset' ? 'detail-confirm' : ''}" type="button" data-asset-delete-selected ${selectedAssetIds.size ? '' : 'disabled'}><i data-lucide="trash-2"></i><span>${pendingBatchDelete === 'asset' ? '确认删除' : '删除所选'}</span></button>
                </div>
            </div>
            ${renderAssetClipboardBar()}
            <div class="content-scroll">
                <div class="asset-grid">
                    ${activeAssetClassFilter ? '' : renderUploadCard(cat)}
                    ${items.map(item => renderAssetCard(item)).join('')}
                    ${items.length ? '' : '<div class="empty-state">当前分组还没有素材，可以上传素材到当前分组。</div>'}
                </div>
            </div>
        </section>
        <aside class="asset-panel asset-detail">${renderAssetDetail(detail)}</aside>
    `;
};
renderAssetDetail = function(item){
    if(!item) return `<div class="panel-head"><div class="panel-title"><strong>素材预览</strong><span>选择一个素材查看详情</span></div></div><div class="detail-scroll"><div class="detail-empty"><i data-lucide="image"></i><span>暂无可预览素材</span></div></div>`;
    const isImage = assetKind(item) === 'image';
    if(assetEditMode && item.id === selectedAssetId){
        return `
...
renderLocalUploadSmartClassTree = function(){ return ''; };
renderLocalUploadDetail = function(item){
    if(!item) return `<div class="panel-head"><div class="panel-title"><strong>素材预览</strong><span>选择一个素材查看详情</span></div></div><div class="detail-scroll"><div class="detail-empty"><i data-lucide="image"></i><span>暂无可预览素材</span></div></div>`;
    const isImage = assetKind(item) === 'image';
    return `
        <div class="panel-head">
...
setStatus = function(msg){
   if(typeof msg === 'string') msg = msg.replace(/去智能画布按 Ctrl\+V 粘贴/g,'去画布按 Ctrl+V 粘贴');
   return _origSetStatus(msg);
};
