...
function showCanvasTextInput({title='输入', message='', value='', placeholder='', okText='确定'}={}){
    return new Promise(resolve => {
        const modal = document.createElement('div');
        modal.className = 'canvas-input-modal open';
        modal.innerHTML = `
            <div class="canvas-input-panel" role="dialog" aria-modal="true">
                <div class="canvas-input-head">
                    <div>
                        <div class="canvas-input-title">${escapeHtml(title)}</div>
                        ${message ? `<div class="canvas-input-message">${escapeHtml(message)}</div>` : ''}
                    </div>
                    <button class="preview-icon-btn" type="button" data-cancel title="关闭"><i data-lucide="x" class="w-4 h-4"></i></button>
                </div>
                <input class="canvas-input-field" type="text" value="${escapeAttr(value)}" placeholder="${escapeAttr(placeholder)}">
                <div class="canvas-input-actions">
                    <button class="error-btn" type="button" data-clear>清空</button>
                    <button class="error-btn" type="button" data-cancel>取消</button>
                    <button class="error-btn primary" type="button" data-ok>${escapeHtml(okText)}</button>
                </div>
            </div>
        `;
        const close = result => {
        ...
        modal.addEventListener('click', event => {
            if(event.target === modal || event.target.closest('[data-cancel]')) close(null);
            else if(event.target.closest('[data-clear]')) close('');
            else if(event.target.closest('[data-ok]')) close(modal.querySelector('.canvas-input-field')?.value ?? '');
        });
        modal.addEventListener('keydown', event => {
            if(event.key === 'Escape') close(null);
            if(event.key === 'Enter') close(modal.querySelector('.canvas-input-field')?.value ?? '');
        });
        document.body.appendChild(modal);
        refreshIcons();
        setTimeout(() => modal.querySelector('.canvas-input-field')?.focus(), 20);
    });
}
...
function canvasVideoFallbackHtml(url, attrs=''){
    ...
    return `<video src="${escapeAttr(src)}" data-url="${escapeAttr(original)}" preload="metadata" playsinline disablepictureinpicture controlslist="nodownload noplaybackrate noremoteplayback"${attrs ? ` ${attrs}` : ''}></video>`;
}
...
function canvasInlineVideoHtml(url, attrs=''){
    const original = canvasOriginalMediaUrl(url);
    const src = canvasDisplayMediaUrl(original);
    return `<video src="${escapeAttr(src)}" data-url="${escapeAttr(original)}" controls playsinline preload="metadata" ${attrs ? ` ${attrs}` : ''}></video>`;
}
function cssAspectRatioValue(value){
...
const canvasAssetUploadBtn = document.getElementById('canvasAssetUploadBtn');
const canvasAssetUploadInput = document.getElementById('canvasAssetUploadInput');
...
const videoCostSummary = document.getElementById('videoCostSummary');
const videoHistoryToggle = document.getElementById('videoHistoryToggle');
const videoHistoryPanel = document.getElementById('videoHistoryPanel');
const videoHistoryCloseBtn = document.getElementById('videoHistoryCloseBtn');
const videoHistoryList = document.getElementById('videoHistoryList');
const videoHistorySub = document.getElementById('videoHistorySub');
...
let videoModels = [];
const VOLC_VIDEO_DURATION_MIN = 4;
const VOLC_VIDEO_DURATION_MAX = 15;
const VOLC_VIDEO_ASPECTS = ['21:9', '16:9', '4:3', '1:1', '3:4', '9:16'];
const VOLC_VIDEO_RESOLUTIONS = ['480p', '720p', '1080p'];
const VOLC_VIDEO_BILLING_FPS = 24;
const VOLC_VIDEO_BILLING_COEFFICIENT = 1;
const VOLC_VIDEO_PRICE_PER_MILLION_TOKENS = 46;
const VOLC_VIDEO_COUNT_MIN = 1;
const VOLC_VIDEO_COUNT_MAX = 8;
const VOLC_VIDEO_DIMENSIONS = {
    '480p': {'21:9':[1120,480], '16:9':[864,480], '4:3':[640,480], '1:1':[480,480], '3:4':[480,640], '9:16':[480,864]},
    '720p': {'21:9':[1680,720], '16:9':[1280,720], '4:3':[960,720], '1:1':[720,720], '3:4':[720,960], '9:16':[720,1280]},
    '1080p': {'21:9':[2520,1080], '16:9':[1920,1080], '4:3':[1440,1080], '1:1':[1080,1080], '3:4':[1080,1440], '9:16':[1080,1920]}
};
let msChatModels = [];
let apiProviders = [];
...
let promptAssetMentionNodeId = '';
let promptAssetMentionTextarea = null;
let promptAssetMentionRange = null;
let promptAssetMentionKind = 'image';
let promptAssetMentionImageGroup = 'all';
let promptAssetMentionLibraryId = '';
let frontNodeId = '';
...
function providerVideoModels(providerId){
    // 不走 providerById（会 fallback 到第一个 provider，造成串台），直接查精确匹配
    const provider = (apiProviders.length ? apiProviders : defaultApiProviders()).find(p => p.id === providerId);
...
function clampVolcVideoCount(value){
    return Math.max(VOLC_VIDEO_COUNT_MIN, Math.min(VOLC_VIDEO_COUNT_MAX, Number(value || 1) || 1));
}
function volcVideoBillingEstimate(input={}){
    const resolution = VOLC_VIDEO_RESOLUTIONS.includes(input.resolution) ? input.resolution : '720p';
    const aspect = VOLC_VIDEO_ASPECTS.includes(input.aspectRatio || input.aspect_ratio) ? (input.aspectRatio || input.aspect_ratio) : '16:9';
    const duration = Math.max(VOLC_VIDEO_DURATION_MIN, Math.min(VOLC_VIDEO_DURATION_MAX, Number(input.duration || 5) || 5));
    const count = clampVolcVideoCount(input.count || input.generationCount || 1);
    const dims = VOLC_VIDEO_DIMENSIONS[resolution]?.[aspect] || VOLC_VIDEO_DIMENSIONS['720p']['16:9'];
    const width = dims[0], height = dims[1];
    const tokensEach = Math.round(width * height * VOLC_VIDEO_BILLING_FPS * duration / 1024 * VOLC_VIDEO_BILLING_COEFFICIENT);
    const tokensTotal = tokensEach * count;
    const yuan = tokensTotal / 1000000 * VOLC_VIDEO_PRICE_PER_MILLION_TOKENS;
    return {width, height, fps:VOLC_VIDEO_BILLING_FPS, coefficient:VOLC_VIDEO_BILLING_COEFFICIENT, duration, count, resolution, aspectRatio:aspect, tokensEach, tokensTotal, yuan};
}
...
function formatVideoBillingShort(estimate){
    if(!estimate) return '';
    return `预计 ¥${formatYuan(estimate.yuan)} / ${estimate.tokensTotal.toLocaleString()} tokens`;
}
function videoBillingFromPayload(payload={}, countOverride=null){
    return volcVideoBillingEstimate({
        duration:payload.duration,
        aspect_ratio:payload.aspect_ratio || payload.aspectRatio,
        resolution:payload.resolution,
        count:countOverride == null ? (payload.count || 1) : countOverride
    });
}
function videoSingleBillingFromEstimate(estimate){
    if(!estimate) return null;
    const tokensTotal = Number(estimate.tokensEach || estimate.tokensTotal || 0);
    return {
        ...estimate,
        count:1,
        tokensTotal,
        yuan: tokensTotal / 1000000 * VOLC_VIDEO_PRICE_PER_MILLION_TOKENS
    };
}
function isVideoGenerationLog(log){
    if(!log) return false;
    if(log.nodeType === 'video') return true;
    return (log.outputs || []).some(item => mediaKindForOutputItem(item) === 'video');
}
function successfulVideoLogs(){
    return (canvas?.logs || []).filter(log => log?.status === 'success' && isVideoGenerationLog(log) && log.billing);
}
...
function sumVideoBilling(logs=[]){
    return logs.reduce((acc, log) => {
        const billing = log.billing || {};
        acc.yuan += Number(billing.yuan || 0);
        acc.tokens += Number(billing.tokensTotal || 0);
        acc.count += Math.max(1, (log.outputs || []).filter(item => mediaKindForOutputItem(item) === 'video').length || Number(billing.count || 1) || 1);
        return acc;
    }, {yuan:0, tokens:0, count:0});
}
function formatCostValue(value){
    return `¥${formatYuan(value)}`;
}
function renderVideoCostSummary(){
    if(!videoCostSummary) return;
    if(!canvas){
        videoCostSummary.classList.remove('visible');
        return;
    }
    const logs = successfulVideoLogs();
    const today = sumVideoBilling(logs.filter(log => isTodayTimestamp(log.createdAt)));
    const project = sumVideoBilling(logs);
    videoCostSummary.classList.add('visible');
    const todayEl = videoCostSummary.querySelector('[data-cost-today]');
    const projectEl = videoCostSummary.querySelector('[data-cost-project]');
    const subEl = videoCostSummary.querySelector('[data-cost-sub]');
    if(todayEl) todayEl.textContent = formatCostValue(today.yuan);
    if(projectEl) projectEl.textContent = formatCostValue(project.yuan);
    if(subEl) subEl.textContent = `${project.count} 条 / ${Math.round(project.tokens).toLocaleString()} tokens`;
}
function outputBillingHtml(meta){
    const billing = meta?.billing || (meta?.run?.nodeType === 'video' ? volcVideoBillingEstimate(meta.run.node || {}) : null);
    if(!billing?.yuan) return '';
    return `<div class="output-cost-pill" title="${escapeAttr(formatVideoBillingShort(billing))}"><i data-lucide="receipt-text"></i><span>预估 ${escapeHtml(formatCostValue(billing.yuan))}</span></div>`;
}
function videoHistoryLogs(){
    return (canvas?.logs || []).filter(log => isVideoGenerationLog(log)).slice(0, 80);
}
function videoHistoryMetaText(log){
    const billing = log.billing || {};
    const parts = [
        new Date(log.createdAt || Date.now()).toLocaleString(window.StudioI18n?.lang() === 'en' ? 'en-US' : 'zh-CN', {month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit'}),
        billing.duration ? `${billing.duration}秒` : '',
        billing.aspectRatio || '',
        billing.resolution || '',
        billing.yuan ? formatCostValue(billing.yuan) : '',
    ].filter(Boolean);
    return parts.join(' · ');
}
function firstVideoOutputUrl(log){
    return outputUrlValue((log.outputs || []).find(item => mediaKindForOutputItem(item) === 'video') || (log.outputs || [])[0]);
}
function reconstructRichPromptFromVideoLog(log){
    let text = String(log?.richPrompt || log?.prompt || '');
    if(!text || /@\[.+?\]\(.+?\)/.test(text)) return text;
    const counters = {image:0, audio:0, video:0};
    const labels = {image:'图片', audio:'音频', video:'视频'};
    (log?.refs || []).forEach(ref => {
        const kind = mediaKindForRef(ref);
        if(!labels[kind] || !ref?.url) return;
        counters[kind] += 1;
        const name = String(ref.name || outputImageName(ref.url) || '').trim();
        const index = counters[kind];
        const token = promptAssetToken(name || `${labels[kind]}${index}`, ref.url);
        const patterns = [
            `${labels[kind]}${index}（${name}）`,
            `${labels[kind]}${index}(${name})`,
            `${labels[kind]}${index}`
        ].filter(Boolean);
        patterns.forEach(pattern => {
            text = text.split(pattern).join(token);
        });
    });
    return text;
}
function renderVideoHistoryPanel(){
    if(!videoHistoryList) return;
    const logs = videoHistoryLogs();
    if(videoHistorySub) videoHistorySub.textContent = `当前画布 ${logs.length} 条视频生成记录`;
    videoHistoryList.innerHTML = logs.length ? logs.map(log => {
        const url = firstVideoOutputUrl(log);
        const thumb = url ? canvasInlineVideoHtml(url, 'muted') : `<div class="video-history-empty-thumb"><i data-lucide="video"></i></div>`;
        const status = log.status === 'failed' ? '失败' : '成功';
        return `<div class="video-history-card ${log.status === 'failed' ? 'failed' : ''}" data-history-id="${escapeAttr(log.id)}">
            <div class="video-history-thumb">${thumb}</div>
            <div class="video-history-info">
                <div class="video-history-title"><span>${escapeHtml(status)}</span><strong>${escapeHtml(log.model || 'Seedance')}</strong></div>
                <div class="video-history-meta">${escapeHtml(videoHistoryMetaText(log))}</div>
                <div class="video-history-prompt" title="${escapeAttr(log.prompt || '')}">${escapeHtml(log.prompt || '无提示词')}</div>
                <div class="video-history-actions">
                    <button type="button" data-history-restore="${escapeAttr(log.id)}"><i data-lucide="copy-plus"></i><span>复制回画布</span></button>
                    <button type="button" data-history-copy="${escapeAttr(log.id)}"><i data-lucide="copy"></i><span>复制提示词</span></button>
                </div>
            </div>
        </div>`;
    }).join('') : `<div class="video-history-empty">还没有视频生成记录</div>`;
    videoHistoryList.querySelectorAll('[data-history-copy]').forEach(btn => {
        btn.onclick = e => {
            e.stopPropagation();
            const log = (canvas?.logs || []).find(item => item.id === btn.dataset.historyCopy);
            if(log?.prompt) navigator.clipboard?.writeText(log.prompt).catch(() => {});
    ...
    videoHistoryList.querySelectorAll('[data-history-restore]').forEach(btn => {
        btn.onclick = e => {
            e.stopPropagation();
            restoreVideoHistoryToCanvas(btn.dataset.historyRestore);
        };
    });
    videoHistoryList.querySelectorAll('.video-history-thumb video').forEach(video => {
        video.addEventListener('play', () => {
            videoHistoryList.querySelectorAll('.video-history-thumb video').forEach(other => {
...
function toggleVideoHistoryPanel(force=null){
    if(!videoHistoryPanel) return;
    const open = force == null ? !videoHistoryPanel.classList.contains('open') : Boolean(force);
    if(open && typeof canvasAssetLibraryOpen !== 'undefined' && canvasAssetLibraryOpen) toggleCanvasAssetLibrary(false);
    videoHistoryPanel.classList.toggle('open', open);
    videoHistoryToggle?.classList.toggle('active', open);
    if(open) renderVideoHistoryPanel();
}
function restoreVideoHistoryToCanvas(logId){
    ...
    const videoH = 360;
    const outputH = 380;
    const layoutW = promptW + gap + videoW + gap + outputW;
    const layoutH = Math.max(promptH + 44, videoH, outputH + 18);
    const target = findAvailableWorkflowPosition(layoutW + 68, layoutH + 108);
    const base = {x:target.x + 34, y:target.y + 64};
    const topY = base.y;
    const promptNode = {
        id:uid('prompt'),
        type:'prompt',
        ...
        w:promptW,
        h:promptH,
        text:reconstructRichPromptFromVideoLog(log)
    };
    const restored = [promptNode];
    const settings = log.videoSettings || {};
    const billing = log.billing || {};
    const videoNode = {
        id:uid('vid'),
        type:'video',
        x:base.x + promptW + gap,
        y:topY,
        w:videoW,
        apiProvider:settings.apiProvider || 'volcengine',
        model:settings.model || (log.model || ''),
        duration:Number(settings.duration || billing.duration || 5),
        aspectRatio:settings.aspectRatio || billing.aspectRatio || '16:9',
        resolution:settings.resolution || billing.resolution || '720p',
        count:1,
        generateAudio:settings.generateAudio !== false,
        watermark:Boolean(settings.watermark),
        multimodal:Boolean(settings.multimodal),
        useFrameRoles:Boolean(settings.useFrameRoles),
        inputs:[]
    };
    restored.push(videoNode);
    const outputItems = (log.outputs || [])
        .filter(item => mediaKindForOutputItem(item) === 'video' || isVideoUrl(outputUrlValue(item)))
        .map(item => {
            ...
            return {
                ...
                url,
                kind:'video',
                aspectRatio:source.aspectRatio || source.aspect_ratio || billing.aspectRatio || settings.aspectRatio || '16:9',
                billing:source.billing || videoSingleBillingFromEstimate(billing) || billing || null,
                run:source.run || {
                    nodeType:'video',
                    node:videoNode,
                    prompt:log.prompt || '',
                    richPrompt:log.richPrompt || '',
                    refs:log.refs || [],
                    billing
                }
            };
    ...
    const outputNode = {
        id:uid('out'),
        type:'output',
        x:videoNode.x + videoW + gap,
        y:topY + 18,
        w:outputW,
        h:outputH,
        images:outputItems
    };
    restored.push(outputNode);
    const boxNodes = restored.map(node => {
        const size = defaultNodeSize(node.type);
        return {x:node.x, y:node.y, w:node.w || size.w || 260, h:node.h || size.h || 180};
    });
    const minX = Math.min(...boxNodes.map(r => r.x));
    const minY = Math.min(...boxNodes.map(r => r.y));
    const maxX = Math.max(...boxNodes.map(r => r.x + r.w));
    const maxY = Math.max(...boxNodes.map(r => r.y + r.h));
    const groupNode = {
        id:uid('grp'),
        type:'group',
        name:'未命名',
        ...
        w:maxX - minX + 68,
        h:maxY - minY + 108,
        items:restored.map(node => node.id)
    };
    const newConnections = [
        {id:uid('c'), from:promptNode.id, to:videoNode.id},
        {id:uid('c'), from:videoNode.id, to:outputNode.id}
    ];
    pushUndo();
    nodes.push(groupNode, ...restored);
    connections.push(...newConnections);
    selected.clear();
    selected.add(groupNode.id);
    render();
    scheduleSave();
    toggleVideoHistoryPanel(false);
    closeCanvasLog();
    requestAnimationFrame(() => focusViewportOnWorldRect(estimatedNodeRect(groupNode)));
}
function renderVideoOptionButtons(values, selected, attr){
    return values.map(value => `<button type="button" class="${value === selected ? 'active' : ''}" data-${attr}="${escapeAttr(value)}">${escapeHtml(value)}</button>`).join('');
}
function setVideoBillingText(pill, yuan, tokens){
    if(!pill) return;
    const yuanEl = pill.querySelector('[data-video-billing-yuan]');
    const tokensEl = pill.querySelector('[data-video-billing-tokens]');
    if(yuanEl) yuanEl.textContent = formatYuan(yuan);
    if(tokensEl) tokensEl.textContent = `${Math.round(Number(tokens || 0)).toLocaleString()}`;
}
function refreshVideoBillingView(wrap, node){
    const pill = wrap?.querySelector?.('.video-billing-pill');
    if(!pill) return;
    const estimate = volcVideoBillingEstimate(node);
    const durationValue = wrap.querySelector('[data-video-duration-value]');
    const countValue = wrap.querySelector('[data-video-count-value]');
    if(durationValue) durationValue.textContent = `${estimate.duration}秒`;
    if(countValue) countValue.textContent = `${estimate.count}条`;
    const fromYuan = Number(pill.dataset.yuan || estimate.yuan);
    const fromTokens = Number(pill.dataset.tokens || estimate.tokensTotal);
    const toYuan = Number(estimate.yuan || 0);
    const toTokens = Number(estimate.tokensTotal || 0);
    pill.dataset.yuan = String(toYuan);
    pill.dataset.tokens = String(toTokens);
    const start = performance.now();
    const duration = 520;
    const ease = t => 1 - Math.pow(1 - t, 3);
    if(pill._billingAnim) cancelAnimationFrame(pill._billingAnim);
    const tick = now => {
        const t = Math.min(1, (now - start) / duration);
        const k = ease(t);
        setVideoBillingText(pill, fromYuan + (toYuan - fromYuan) * k, fromTokens + (toTokens - fromTokens) * k);
        if(t < 1) pill._billingAnim = requestAnimationFrame(tick);
    };
    pill._billingAnim = requestAnimationFrame(tick);
}
...
function focusViewportOnWorldRect(rect){
    if(!rect) return;
    const boardRect = board.getBoundingClientRect();
    const pad = 180;
    const fitScale = Math.min(
        1,
        Math.max(0.08, (boardRect.width - 120) / Math.max(1, Number(rect.w || 1) + pad)),
        Math.max(0.08, (boardRect.height - 120) / Math.max(1, Number(rect.h || 1) + pad))
    );
    viewport.scale = Math.max(0.12, Math.min(safeViewportScale(viewport.scale), fitScale));
...
function addAudioNode(point){
    const p = point || defaultPoint(-120, 0);
    return addNode({id:uid('aud'), type:'image', x:p.x, y:p.y, url:'', name:'Audio', mediaKind:'audio'});
}
...
function addGroupNode(point){
    const p = point || defaultPoint(40, 0);
    return addNode({id:uid('grp'), type:'group', name:nextGroupName(), x:p.x, y:p.y, w:300, h:220, items:[]});
}
function pickMediaForNode(nodeId, accept='image/*,video/*,audio/*'){
    const input = document.createElement('input');
    input.type = 'file';
    input.accept = accept;
    input.multiple = true;
...
function pickAudioForNode(nodeId){
    pickMediaForNode(nodeId, 'audio/*,.mp3,.wav,.m4a,.aac,.ogg,.flac');
}
...
function linkCreateOptions(state){
    ...
    if(CANVAS_GENERATOR_TYPES.includes(node.type) || node.type === 'llm'){
        return [
            ...
            {type:'audio', label:'上传音频', icon:'file-audio'},
...
function openImageNodeMenu(nodeId, clientX, clientY){
    ...
    imageNodeMenu.querySelector('[data-image-replace]').onclick = e => {
        e.stopPropagation();
        closeImageNodeMenu();
        mediaKindForNode(node) === 'audio' ? pickAudioForNode(nodeId) : pickImageForNode(nodeId);
    };
    refreshIcons();
}
...
function openCanvasNodeActionMenu(nodeId, clientX, clientY){
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return false;
    const groupableCount = selectedGroupableNodes().length;
    const canGroup = groupableCount >= 1;
    const canRenameGroup = node.type === 'group';
    if(!canGroup && !canRenameGroup) return false;
    closeCreateMenu();
    imageNodeMenu.classList.remove('output-node-menu');
    imageNodeMenu.innerHTML = `
        ${canGroup ? `<button class="menu-btn" data-node-group-selected><i data-lucide="group" class="w-4 h-4"></i><span>打组选中节点</span></button>` : ''}
        ${canRenameGroup ? `<button class="menu-btn" data-node-rename-group="${escapeAttr(nodeId)}"><i data-lucide="pencil" class="w-4 h-4"></i><span>重命名组</span></button>` : ''}
        ${canRenameGroup ? `<button class="menu-btn" data-node-ungroup="${escapeAttr(nodeId)}"><i data-lucide="ungroup" class="w-4 h-4"></i><span>取消打组</span></button>` : ''}
    `;
    imageNodeMenu.style.left = `${clientX}px`;
    imageNodeMenu.style.top = `${clientY}px`;
    imageNodeMenu.classList.add('open');
    const groupBtn = imageNodeMenu.querySelector('[data-node-group-selected]');
    if(groupBtn){
        groupBtn.onclick = e => {
            e.stopPropagation();
            closeImageNodeMenu();
    ...
    const renameBtn = imageNodeMenu.querySelector('[data-node-rename-group]');
    if(renameBtn){
        renameBtn.onclick = e => {
            e.stopPropagation();
            closeImageNodeMenu();
            renameGroupNode(renameBtn.dataset.nodeRenameGroup || nodeId);
        };
    }
    const ungroupBtn = imageNodeMenu.querySelector('[data-node-ungroup]');
    if(ungroupBtn){
        ungroupBtn.onclick = e => {
            e.stopPropagation();
            closeImageNodeMenu();
            ungroupNode(ungroupBtn.dataset.nodeUngroup || nodeId);
        };
    }
    refreshIcons();
    return true;
...
function currentCanvasMediaLinks(node){
    const refs = orderedSources(node, generatorSources(node)).flatMap(src => src.refs || [])
        .filter(ref => ref?.url && ['image','video','audio'].includes(mediaKindForRef(ref)));
    return refs.map(ref => {
        const uploaded = tempShUploadedUrlForNode(node, ref.url);
        return uploaded && uploaded !== ref.url ? uploaded : '';
    }).filter(Boolean);
}
function videoTemporaryPublicLinks(node){
    const seen = new Set();
    return (node?.tempShLinks || [])
        .filter(item => item?.url && !item.manual)
        .filter(item => {
...
function renderVideoTemporaryPublicLinks(node){
    const links = videoTemporaryPublicLinks(node);
    if(!links.length) return '';
    return `<div class="video-temp-public-links">
        <div class="video-temp-public-title"><i data-lucide="link"></i><span>临时公网链接</span></div>
        ${links.map((item, index) => `
            <div class="video-temp-public-row">
                <span class="video-temp-public-name">${escapeHtml(item.name || item.kind || `素材${index + 1}`)}</span>
                <input readonly value="${escapeAttr(item.url)}" title="${escapeAttr(item.url)}">
                <button type="button" data-copy-temp-public="${escapeAttr(item.url)}" title="复制链接"><i data-lucide="copy"></i></button>
                <button type="button" data-open-temp-public="${escapeAttr(item.url)}" title="打开链接"><i data-lucide="external-link"></i></button>
            </div>
        `).join('')}
        <div class="video-temp-public-note">临时公网链接通常约 3 天有效，适合给火山接口临时读取本地视频/图片。</div>
    </div>`;
}
...
async function uploadCanvasMediaRefToCloud(node, ref){
    ...
    if(!response.ok) throw new Error(await responseErrorMessage(response, '上传到临时公网失败'));
    const data = await response.json();
    const uploadedUrl = data.url || '';
    if(!uploadedUrl) throw new Error('云端没有返回链接');
    node.tempShLinks = [
        ...(node.tempShLinks || []).filter(item => item?.source !== ref.url),
        {source:ref.url, url:uploadedUrl, expires:data.expires || '3 days', kind, service:data.service || '', name:data.name || ''}
    ];
...
async function uploadCanvasVideosToCloud(nodeId){
    ...
    if(!localRefs.length){
        showErrorModal('没有需要上传到临时公网的本地图片或视频', '上传到临时公网');
        return [];
    }
    ...
    try {
        ...
        showErrorModal(`已上传 ${urls.length} 个媒体文件到临时公网，首个链接已复制。链接约 3 天有效。`, '上传到临时公网');
        return urls;
    } catch(e) {
        node.tempShUploading = false;
        refreshNodes([node.id]);
...
async function setCanvasManualVideoUrl(nodeId){
    ...
    const value = await showCanvasTextInput({
        title:'输入媒体网址',
        message:'支持 http/https 临时公网链接，或 asset:// 火山素材 URI。',
...
function render(){
    ...
    refreshOutputTimer();
    renderVideoCostSummary();
    if(videoHistoryPanel?.classList.contains('open')) renderVideoHistoryPanel();
}
...
function captureOutputScrolls(){
    const state = new Map();
    // 提示词富文本在 render / @素材插入后也要保持用户当前阅读位置。
    nodesEl.querySelectorAll('.prompt-node').forEach(el => {
        const editor = el.querySelector('.prompt-rich-editor');
        if(editor) state.set('prompt:' + el.dataset.id, { top:editor.scrollTop, left:editor.scrollLeft });
    });
    // output 节点滚动位置
    ...
    requestAnimationFrame(() => {
        state.forEach((pos, key) => {
            if(key.startsWith('out:')){
                ...
                const editor = nodesEl.querySelector(`.prompt-node[data-id="${CSS.escape(id)}"] .prompt-rich-editor`);
                if(editor){ editor.scrollTop = pos.top || 0; editor.scrollLeft = pos.left || 0; }
...
function renderNode(node){
    ...
    const canFloatFront = node.type !== 'group' && node.type !== 'promptGroup';
    el.className = `node ${node.type}-node ${node.url ? 'has-image' : ''} ${hasFixedSize ? 'sized' : ''} ${selected.has(node.id) ? 'selected' : ''} ${canFloatFront && frontNodeId === node.id ? 'node-front' : ''}`;
    ...
    el.oncontextmenu = e => {
        e.preventDefault();
        e.stopPropagation();
        if(selected.size > 1 || node.type === 'group'){
            if(openCanvasNodeActionMenu(node.id, e.clientX, e.clientY)) return;
        }
        if(node.type === 'output') openOutputNodeMenu(node.id, e.clientX, e.clientY);
        else if(CANVAS_GENERATOR_TYPES.includes(node.type)) openGeneratorNodeMenu(node.id, e.clientX, e.clientY);
        else openCanvasNodeActionMenu(node.id, e.clientX, e.clientY);
    };
    const title = node.type === 'image' ? 'Image' : node.type === 'prompt' ? 'Prompt' : node.type === 'loop' ? tr('canvas.loopNode') : node.type === 'promptGroup' ? 'Prompts' : node.type === 'group' ? (node.name || tr('canvas.group')) : node.type === 'output' ? 'Output' : node.type === 'llm' ? 'LLM' : node.type === 'comfy' ? 'ComfyUI' : node.type === 'ltxDirector' ? tr('canvas.ltxDirector') : node.type === 'rh' ? 'RunningHub' : node.type === 'msgen' ? tr('canvas.modelscopeGenerate') : node.type === 'video' ? tr('canvas.videoGenerateNode') : tr('canvas.apiGenerate');
    ...
    if(node.type === 'image') {
        if(node.url) {
            ...
            const emptyKind = mediaKindForNode(node);
            const emptyIcon = emptyKind === 'audio' ? 'file-audio' : 'image-plus';
            const emptyText = emptyKind === 'audio' ? '点击上传音频，或拖入音频素材' : tr('canvas.clickDragPasteImage');
        body.innerHTML = `<div class="blank-image"><i data-lucide="${emptyIcon}" class="w-7 h-7"></i><div class="text-[11px] font-bold">${emptyText}</div></div>`;
            const blank = body.querySelector('.blank-image');
            blank.onclick = () => emptyKind === 'audio' ? pickAudioForNode(node.id) : pickImageForNode(node.id);
    ...
    if(node.type === 'prompt') {
        migratePromptAssetRefsToText(node);
        const templateActive = promptTemplateModal?.classList.contains('open') && promptTemplateNodeId === node.id;
        body.innerHTML = `<div class="prompt-editor"><div class="prompt-toolbar"><div class="prompt-toolbar-left"><button class="prompt-template-btn ${templateActive ? 'active' : ''}" type="button" data-prompt-template-open data-prompt-template-node-id="${escapeAttr(node.id)}" aria-pressed="${templateActive ? 'true' : 'false'}" title="${escapeAttr(tr('canvas.promptTemplateLibrary'))}"><i data-lucide="library"></i><span>${escapeHtml(tr('canvas.promptTemplateShort'))}</span></button><button class="prompt-template-btn" type="button" data-prompt-asset-mention data-prompt-asset-node-id="${escapeAttr(node.id)}" onclick="openPromptAssetMentionFromButton(this, event)" title="@ 资产库"><i data-lucide="at-sign"></i><span>@素材</span></button></div>${promptCounterHtml(promptAssetDisplayText(node.text || ''))}</div><div class="prompt-rich-editor" contenteditable="true" spellcheck="false" data-placeholder="${escapeAttr(tr('canvas.promptPlaceholder'))}">${promptRichEditorHtml(node.text || '')}</div></div>`;
        const editor = body.querySelector('.prompt-rich-editor');
        const templateBtn = body.querySelector('[data-prompt-template-open]');
        const mentionBtn = body.querySelector('[data-prompt-asset-mention]');
        ...
        mentionBtn.onclick = e => {
            e.preventDefault();
            e.stopPropagation();
            const caret = Number.isFinite(Number(editor.dataset.lastCaret))
                ? Number(editor.dataset.lastCaret)
                : promptRichEditorCaretOffset(editor);
            openPromptAssetMention(node.id, editor, {start:caret, end:caret});
        };
        bindScrollableText(editor);
        const rememberPromptCaret = () => {
            editor.dataset.lastCaret = String(promptRichEditorCaretOffset(editor));
        };
        const syncPromptEditorValue = () => {
            node.text = promptTextFromRichEditor(editor);
            rememberPromptCaret();
            refreshPromptCounter(body, promptAssetDisplayText(node.text));
            scheduleSave();
            scheduleGeneratorInputSync();
        };
        let mentionAutoOpenPending = false;
        const openMentionFromTypedAt = () => {
            if(mentionAutoOpenPending || document.getElementById('promptAssetMentionModal')) return;
            const caret = promptRichEditorCaretOffset(editor);
            if(node.text.slice(Math.max(0, caret - 1), caret) !== '@') return;
            mentionAutoOpenPending = true;
            Promise.resolve(openPromptAssetMention(node.id, editor, {start:Math.max(0, caret - 1), end:caret}))
                .finally(() => { mentionAutoOpenPending = false; });
        };
        editor.addEventListener('keyup', e => {
            rememberPromptCaret();
            if(e.key === '@') openMentionFromTypedAt();
        });
        editor.addEventListener('mouseup', rememberPromptCaret);
        editor.addEventListener('focus', rememberPromptCaret);
        editor.addEventListener('paste', e => {
            const plainText = e.clipboardData?.getData('text/plain');
            if(plainText == null) return;
            e.preventDefault();
            insertPromptPlainTextAtSelection(editor, plainText);
            syncPromptEditorValue();
        });
        editor.oninput = e => {
            syncPromptEditorValue();
            const caret = promptRichEditorCaretOffset(editor);
            const insertedAtTrigger = !e.isComposing
                && !String(e.inputType || '').startsWith('delete')
                && e.inputType !== 'insertFromPaste'
                && node.text.slice(Math.max(0, caret - 1), caret) === '@';
            if(insertedAtTrigger) queueMicrotask(openMentionFromTypedAt);
        };
    }
    ...
    if(node.type === 'group') {
        ...
        const videoCount = items.filter(n => n.type === 'video').length;
        const outputCount = items.filter(n => n.type === 'output').length;
        const otherCount = Math.max(0, items.length - imgCount - promptCount - videoCount - outputCount);
        const parts = [];
        if(imgCount) parts.push(`${imgCount} ${tr('canvas.imageCount')}`);
        if(promptCount) parts.push(`${promptCount} ${tr('canvas.promptCount')}`);
        if(videoCount) parts.push(`${videoCount} 视频生成`);
        if(outputCount) parts.push(`${outputCount} Output`);
        if(otherCount) parts.push(`${otherCount} 节点`);
        const text = parts.length ? `${parts.join(' · ')} ${tr('canvas.grouped')}` : tr('canvas.groupEmpty');
        body.innerHTML = `<div class="text-[11px] text-gray-400">${escapeHtml(text)}</div>`;
...
function outputAspectFromMeta(meta){
    return cssAspectRatioValue(meta?.aspectRatio || meta?.aspect_ratio || meta?.ratio || meta?.run?.request?.aspect_ratio || '');
}
function syncOutputVideoFrame(wrap){
    if(!wrap?.classList?.contains('output-video-wrap')) return;
    const apply = video => {
        if(!video) return;
        video.defaultMuted = false;
        video.muted = false;
        ...
        if(video.videoWidth && video.videoHeight){
            wrap.style.setProperty('--output-aspect', `${video.videoWidth} / ${video.videoHeight}`);
            wrap.classList.toggle('output-video-portrait', video.videoHeight > video.videoWidth);
            wrap.classList.toggle('output-video-landscape', video.videoWidth >= video.videoHeight);
        }
    };
    const video = wrap.querySelector('video');
    if(video){
        apply(video);
        video.addEventListener('loadedmetadata', () => apply(video), {once:true});
    }
}
function bindOutputWrap(wrap, node){
    ...
    if(video){
        video.onclick = e => {
            if(wrap.classList.contains('output-video-wrap')){
                e.stopPropagation();
                return;
            }
            e.stopPropagation();
...
function defaultNodeSize(type){
    ...
    if(type === 'output') return {w:520, h:0};
    return {w:260, h:0};
}
...
function promptAssetMentionRefs(text=''){
    const refs = [];
    const seen = new Set();
    String(text || '').replace(/@\[([^\]]{1,120})\]\(([^)\s]+)\)/g, (_, name, url) => {
        const cleanUrl = String(url || '').trim();
        if(!cleanUrl || seen.has(cleanUrl)) return '';
        seen.add(cleanUrl);
        refs.push({url:cleanUrl, name:String(name || outputImageName(cleanUrl) || 'asset').trim(), kind:mediaKindForRef({url:cleanUrl})});
        return '';
    });
    return refs;
}
function promptAssetDisplayText(text=''){
    return String(text || '').replace(/@\[([^\]]{1,120})\]\(([^)\s]+)\)/g, (_, name) => `@${String(name || 'asset').trim()}`);
}
function promptAssetToken(name, url){
    return `@[${String(name || outputImageName(url) || 'asset').replace(/[\[\]\n\r]/g, ' ').trim()}](${url})`;
}
function promptInlineAssetHtml(name, url){
    const cleanUrl = String(url || '').trim();
    const cleanName = String(name || outputImageName(cleanUrl) || 'asset').trim();
    const kind = mediaKindForRef({url:cleanUrl});
    const token = promptAssetToken(cleanName, cleanUrl);
    let thumb = '';
    if(kind === 'audio') thumb = '<span class="prompt-inline-asset-thumb prompt-inline-asset-audio"><i data-lucide="audio-lines"></i></span>';
    else if(kind === 'video') thumb = `<span class="prompt-inline-asset-thumb">${canvasVideoPreviewHtml(cleanUrl, 80)}</span>`;
    else thumb = `<span class="prompt-inline-asset-thumb">${canvasPreviewImgHtml(cleanUrl, 80)}</span>`;
    return `<span class="prompt-inline-asset prompt-inline-asset-${escapeAttr(kind)}" contenteditable="false" data-token="${escapeAttr(token)}" data-url="${escapeAttr(cleanUrl)}" data-name="${escapeAttr(cleanName)}" data-kind="${escapeAttr(kind)}">${thumb}<span class="prompt-inline-asset-label">@${escapeHtml(cleanName)}</span></span>`;
}
function promptRichEditorHtml(text=''){
    const source = String(text || '');
    let out = '';
    let last = 0;
    source.replace(/@\[([^\]]{1,120})\]\(([^)\s]+)\)/g, (match, name, url, offset) => {
        out += escapeHtml(source.slice(last, offset)).replace(/\n/g, '<br>');
        out += promptInlineAssetHtml(name, url);
...
const PROMPT_RICH_BLOCK_TAGS = new Set(['ADDRESS','ARTICLE','ASIDE','BLOCKQUOTE','DIV','FIGCAPTION','FIGURE','FOOTER','H1','H2','H3','H4','H5','H6','HEADER','LI','MAIN','NAV','OL','P','PRE','SECTION','UL']);
function promptRichNodeIsBlock(node){
    return Boolean(node?.nodeType === 1 && PROMPT_RICH_BLOCK_TAGS.has(node.tagName));
}
function promptTextFromRichChildren(parent){
    const children = Array.from(parent?.childNodes || []);
    let out = '';
    children.forEach((child, index) => {
        const part = promptTextFromRichNode(child);
        const previous = children[index - 1];
        if(index > 0 && (promptRichNodeIsBlock(previous) || promptRichNodeIsBlock(child)) && out && !out.endsWith('\n') && !part.startsWith('\n')) out += '\n';
        out += part;
    });
    return out;
}
function promptTextFromRichNode(node){
    if(!node) return '';
    if(node.nodeType === 3) return node.textContent || '';
    if(node.nodeType !== 1) return '';
    const el = node;
    if(el.classList?.contains('prompt-inline-asset')) return el.dataset.token || promptAssetToken(el.dataset.name || 'asset', el.dataset.url || '');
    if(el.tagName === 'BR') return '\n';
    return promptTextFromRichChildren(el);
}
function promptTextFromRichEditor(editor){
    return promptTextFromRichChildren(editor).replace(/\u00a0/g, ' ');
}
function insertPromptPlainTextAtSelection(editor, text=''){
    if(!editor) return;
...
function promptRichEditorCaretOffset(editor){
    const sel = window.getSelection?.();
    if(!editor || !sel || !sel.rangeCount) return promptTextFromRichEditor(editor).length;
    const current = sel.getRangeAt(0);
    if(!editor.contains(current.endContainer)) return promptTextFromRichEditor(editor).length;
    try {
        const beforeCaret = current.cloneRange();
        beforeCaret.setStart(editor, 0);
        const fragment = beforeCaret.cloneContents();
        const probe = document.createElement('div');
        probe.appendChild(fragment);
        return promptTextFromRichEditor(probe).length;
    } catch(_) {
        return promptTextFromRichEditor(editor).length;
    }
}
function setPromptRichEditorCaret(editor, targetOffset){
    if(!editor) return;
    ...
    const place = (node) => {
        ...
        if(node.nodeType === 1 && node.classList?.contains('prompt-inline-asset')){
            const len = node.dataset.token?.length || 0;
            if(remaining <= len){
...
function promptAssetMentionItems(){
    const items = [];
    const push = (item, category=null, library=null) => {
        const kind = canvasAssetItemKind(item);
        if(!item?.url || !['image','video','audio'].includes(kind)) return;
        const categoryName = String(category?.name || '').trim();
        const categoryId = String(category?.id || '').trim();
        let imageGroup = 'other';
        if(/人物|角色|模特|演员|character|people|person/i.test(`${categoryName} ${categoryId}`)) imageGroup = 'people';
        else if(/场景|环境|地点|scene|environment|location/i.test(`${categoryName} ${categoryId}`)) imageGroup = 'scene';
        else if(/道具|物品|物体|产品|prop|object|product/i.test(`${categoryName} ${categoryId}`)) imageGroup = 'prop';
        items.push({
            id:item.id || item.url,
            name:item.name || outputImageName(item.url) || kind,
            url:item.url,
            kind,
            categoryName,
            categoryId,
    ...
    canvasAssetLibraries().forEach(lib => (lib.categories || []).forEach(cat => (cat.items || []).forEach(item => push(item, cat, lib))));
    (localCanvasAssetLibrary.items || []).forEach(item => push(item, {id:'local', name:'本地素材'}, {id:LOCAL_CANVAS_ASSET_LIBRARY_ID, name:'本地素材'}));
    const seen = new Set();
    return items.filter(item => {
        const key = `${item.libraryId || 'library'}:${item.url}`;
        if(seen.has(key)) return false;
        seen.add(key);
        return true;
    });
}
function promptAssetMentionLabel(kind){
    if(kind === 'audio') return '音频';
    if(kind === 'video') return '视频';
    return '图片';
}
function normalizePromptAssetRefs(refs=[]){
    const seen = new Set();
    return (Array.isArray(refs) ? refs : []).map(ref => {
        ...
        return {
            url,
            name:String(ref?.name || outputImageName(url) || 'asset').trim(),
            kind:['image','video','audio'].includes(kind) ? kind : mediaKindForRef({url})
        };
    }).filter(Boolean);
}
function promptNodeAssetRefs(node){
    return normalizePromptAssetRefs([...(node?.assetRefs || []), ...promptAssetMentionRefs(node?.text || '')]);
}
function migratePromptAssetRefsToText(node){
    if(!node || node.type !== 'prompt' || !Array.isArray(node.assetRefs) || !node.assetRefs.length) return;
    const existing = new Set(promptAssetMentionRefs(node.text || '').map(ref => ref.url));
    const refs = normalizePromptAssetRefs(node.assetRefs).filter(ref => !existing.has(ref.url));
    if(refs.length){
        const suffix = refs.map(ref => promptAssetToken(ref.name, ref.url)).join(' ');
        node.text = `${String(node.text || '').trimEnd()}${node.text ? ' ' : ''}${suffix}`;
    }
    node.assetRefs = [];
}
function promptAssetRefThumbHtml(ref){
    const kind = ref?.kind || mediaKindForRef(ref);
    if(kind === 'audio'){
        return `<div class="prompt-asset-ref-thumb prompt-asset-ref-audio"><i data-lucide="file-audio"></i></div>`;
    }
    if(kind === 'video'){
        return `<div class="prompt-asset-ref-thumb">${canvasVideoPreviewHtml(ref.url, 160)}</div>`;
    }
    return `<div class="prompt-asset-ref-thumb">${canvasPreviewImgHtml(ref.url, 160)}</div>`;
}
function promptAssetMentionThumbHtml(item){
    if(item.kind === 'audio'){
        return `<div class="prompt-asset-mention-thumb prompt-asset-mention-audio"><i data-lucide="file-audio"></i></div>`;
    }
    if(item.kind === 'video'){
        return `<div class="prompt-asset-mention-thumb">${canvasVideoPreviewHtml(item.url, 160)}</div>`;
    }
    return `<div class="prompt-asset-mention-thumb">${canvasPreviewImgHtml(item.url, 160)}</div>`;
}
function renderPromptAssetRefsHtml(node){
    const refs = promptNodeAssetRefs(node);
    if(!refs.length) return '';
    return `<div class="prompt-asset-ref-strip">${refs.map(ref => `
        <div class="prompt-asset-ref-card" title="${escapeAttr(ref.name || ref.url)}">
            ${promptAssetRefThumbHtml(ref)}
            <div class="prompt-asset-ref-meta">
                <span>${escapeHtml(ref.name || 'asset')}</span>
                <small>${promptAssetMentionLabel(ref.kind)}</small>
            </div>
            <button type="button" data-remove-prompt-asset="${escapeAttr(ref.url)}" title="移除"><i data-lucide="x"></i></button>
        </div>
    `).join('')}</div>`;
}
function removePromptAssetRef(nodeId, url){
    const node = nodes.find(n => n.id === nodeId && n.type === 'prompt');
    if(!node || !url) return;
    node.assetRefs = normalizePromptAssetRefs(node.assetRefs || []).filter(ref => ref.url !== url);
    render();
    scheduleSave();
    scheduleGeneratorInputSync();
}
async function openPromptAssetMention(nodeId, textarea=null, replaceRange=null){
    promptAssetMentionNodeId = nodeId || '';
    promptAssetMentionTextarea = textarea || promptAssetMentionTextarea;
    promptAssetMentionRange = replaceRange || (
        promptAssetMentionTextarea
            ? (
                promptAssetMentionTextarea.classList?.contains('prompt-rich-editor')
                    ? (() => {
                        const caret = promptRichEditorCaretOffset(promptAssetMentionTextarea);
                        return {start:caret, end:caret};
                    })()
                    : {start:promptAssetMentionTextarea.selectionStart || 0, end:promptAssetMentionTextarea.selectionEnd || 0}
            )
            : null
    );
    promptAssetMentionKind = 'image';
    promptAssetMentionImageGroup = 'all';
    if(!canvasAssetLibrary?.libraries?.length) await loadCanvasAssetLibrary({renderPanel:false});
    if(!promptAssetMentionLibraryId) promptAssetMentionLibraryId = activeCanvasAssetLibraryId || canvasAssetLibrary.active_library_id || canvasAssetLibraries()[0]?.id || '';
    renderPromptAssetMentionModal('');
}
function openPromptAssetMentionFromButton(button, event){
    event?.preventDefault?.();
    event?.stopPropagation?.();
    const nodeId = button?.dataset?.promptAssetNodeId || button?.closest?.('.node')?.dataset?.id || '';
    const textarea = button?.closest?.('.prompt-editor')?.querySelector?.('.prompt-rich-editor, textarea') || null;
    if(nodeId) openPromptAssetMention(nodeId, textarea);
}
function closePromptAssetMentionModal(){
    document.getElementById('promptAssetMentionModal')?.remove();
    promptAssetMentionNodeId = '';
    promptAssetMentionTextarea = null;
    promptAssetMentionRange = null;
    promptAssetMentionKind = 'image';
    promptAssetMentionImageGroup = 'all';
    promptAssetMentionLibraryId = '';
}
function renderPromptAssetMentionModal(query=''){
    document.getElementById('promptAssetMentionModal')?.remove();
    const q = String(query || '').trim().toLowerCase();
    const mentionLibraries = canvasAssetSourceLibraries();
    if(!promptAssetMentionLibraryId || !mentionLibraries.some(lib => lib.id === promptAssetMentionLibraryId)){
        promptAssetMentionLibraryId = activeCanvasAssetLibraryId || canvasAssetLibrary.active_library_id || canvasAssetLibraries()[0]?.id || mentionLibraries[0]?.id || '';
    }
    const allItems = promptAssetMentionItems().filter(item => !promptAssetMentionLibraryId || item.libraryId === promptAssetMentionLibraryId);
    const imageGroups = [
        {id:'all', label:'全部'},
        {id:'people', label:'人物'},
        {id:'scene', label:'场景'},
        {id:'prop', label:'道具'},
        {id:'other', label:'其他'}
    ].filter(group => group.id === 'all' || allItems.some(item => item.kind === 'image' && item.imageGroup === group.id));
    const items = allItems.filter(item => {
        if(item.kind !== promptAssetMentionKind) return false;
        if(item.kind === 'image' && promptAssetMentionImageGroup !== 'all' && item.imageGroup !== promptAssetMentionImageGroup) return false;
        return !q || [item.name, item.url, item.kind, item.categoryName, item.libraryName].join(' ').toLowerCase().includes(q);
    });
    const modal = document.createElement('div');
    modal.id = 'promptAssetMentionModal';
    modal.className = 'prompt-asset-mention-modal';
    modal.innerHTML = `
        <div class="prompt-asset-mention-panel">
            <div class="prompt-asset-mention-head">
                <strong>@ 资产库</strong>
                <button type="button" data-mention-close title="关闭"><i data-lucide="x"></i></button>
            </div>
            <label class="prompt-asset-mention-search"><i data-lucide="search"></i><input type="search" value="${escapeAttr(query)}" placeholder="搜索图片、音频、视频素材"></label>
            <select class="prompt-asset-mention-library" aria-label="选择资产库">
                ${mentionLibraries.map(lib => `<option value="${escapeAttr(lib.id)}" ${lib.id === promptAssetMentionLibraryId ? 'selected' : ''}>${escapeHtml(lib.name || '资产库')}</option>`).join('')}
            </select>
            <div class="prompt-asset-mention-kind-tabs">
                ${[
                    {id:'image', label:'图片', count:allItems.filter(item => item.kind === 'image').length},
                    {id:'audio', label:'音频', count:allItems.filter(item => item.kind === 'audio').length},
                    {id:'video', label:'视频', count:allItems.filter(item => item.kind === 'video').length}
                ].map(tab => `<button type="button" class="${promptAssetMentionKind === tab.id ? 'active' : ''}" data-mention-kind-filter="${tab.id}"><span>${tab.label}</span><small>${tab.count}</small></button>`).join('')}
            </div>
            ${promptAssetMentionKind === 'image' ? `<div class="prompt-asset-mention-subtabs">
                ${imageGroups.map(group => `<button type="button" class="${promptAssetMentionImageGroup === group.id ? 'active' : ''}" data-mention-image-group="${group.id}">${group.label}</button>`).join('')}
            </div>` : ''}
            <div class="prompt-asset-mention-list">
                ${items.length ? items.map(item => `
                    <button type="button" data-mention-asset="${escapeAttr(item.url)}" data-mention-name="${escapeAttr(item.name)}" data-mention-kind="${escapeAttr(item.kind)}">
                        ${promptAssetMentionThumbHtml(item)}
                        <span>${escapeHtml(item.name || 'asset')}</span>
                        <small>${item.kind === 'image' && item.categoryName ? escapeHtml(item.categoryName) : promptAssetMentionLabel(item.kind)}</small>
                    </button>
                `).join('') : '<div class="prompt-asset-mention-empty">当前资产库里还没有可引用的图片、音频或视频</div>'}
            </div>
        </div>
    `;
    modal.addEventListener('click', event => {
        if(event.target === modal || event.target.closest('[data-mention-close]')) closePromptAssetMentionModal();
    });
    modal.querySelector('input')?.addEventListener('input', event => renderPromptAssetMentionModal(event.target.value || ''));
    modal.querySelector('.prompt-asset-mention-library')?.addEventListener('change', event => {
        promptAssetMentionLibraryId = event.target.value || '';
        promptAssetMentionImageGroup = 'all';
        renderPromptAssetMentionModal(query);
    });
    modal.querySelectorAll('[data-mention-kind-filter]').forEach(btn => {
        btn.addEventListener('click', () => {
            promptAssetMentionKind = btn.dataset.mentionKindFilter || 'image';
            promptAssetMentionImageGroup = 'all';
            renderPromptAssetMentionModal(query);
        });
    });
    modal.querySelectorAll('[data-mention-image-group]').forEach(btn => {
        btn.addEventListener('click', () => {
            promptAssetMentionImageGroup = btn.dataset.mentionImageGroup || 'all';
            renderPromptAssetMentionModal(query);
        });
    });
    modal.querySelectorAll('[data-mention-asset]').forEach(btn => {
        btn.addEventListener('click', () => insertPromptAssetMention(btn.dataset.mentionName || 'asset', btn.dataset.mentionAsset || ''));
    });
    document.body.appendChild(modal);
    modal.querySelector('input')?.focus();
    refreshIcons();
}
function insertPromptAssetMention(name, url){
    const node = nodes.find(n => n.id === promptAssetMentionNodeId && n.type === 'prompt');
    if(!node || !url) return;
    const textarea = promptAssetMentionTextarea;
    const isRich = textarea?.classList?.contains('prompt-rich-editor');
    const current = textarea ? (isRich ? promptTextFromRichEditor(textarea) : textarea.value) : (node.text || '');
    const start = Math.max(0, Math.min(current.length, Number(promptAssetMentionRange?.start ?? current.length)));
    const end = Math.max(start, Math.min(current.length, Number(promptAssetMentionRange?.end ?? start)));
    const token = promptAssetToken(name, url);
    ...
    if(textarea){
        ...
        if(isRich){
            ...
            textarea.innerHTML = promptRichEditorHtml(next);
            textarea.focus();
            setPromptRichEditorCaret(textarea, caret);
        ...
        refreshPromptCounter(textarea.closest('.prompt-editor'), promptAssetDisplayText(next));
    }
    closePromptAssetMentionModal();
    scheduleSave();
    scheduleGeneratorInputSync();
}
...
function canvasAssetCategories(){
    return (activeCanvasAssetLibrary()?.categories || canvasAssetLibrary.categories || []).filter(cat => {
        ...
        return type === 'image' || type === 'media' || type === 'video' || type === 'audio' || type === 'workflow';
    });
}
function canvasMediaCategories(){
    return (activeCanvasAssetLibrary()?.categories || canvasAssetLibrary.categories || []).filter(cat => {
        const type = String(cat.type || 'image').toLowerCase();
        return type === 'image' || type === 'media' || type === 'video' || type === 'audio';
    });
}
...
function renderCanvasAssetLibrary(){
    ...
    if(canvasAssetUploadBtn) {
        canvasAssetUploadBtn.disabled = localMode || !cat || catType === 'workflow';
        canvasAssetUploadBtn.title = localMode ? '本地素材请在资产库管理中上传' : (catType === 'workflow' ? '工作流分组不支持直接上传媒体' : '上传资产');
    }
    ...
    `).join('') : `<div class="canvas-asset-empty">${escapeHtml(localMode ? '暂无本地素材，请在资产库管理中上传' : '当前分组还没有资产')}</div>`;
...
function toggleCanvasAssetLibrary(open=!canvasAssetLibraryOpen){
    ...
    if(canvasAssetLibraryOpen && videoHistoryPanel?.classList.contains('open')) toggleVideoHistoryPanel(false);
...
async function addUrlToCanvasAssetLibrary(url, name=''){
    if(canvasAssetLibraryIsLocal()){ setStatus('本地素材请在资产库管理中上传'); return; }
    const cat = activeCanvasAssetCategory();
...
async function uploadFilesToActiveCanvasAssetLibrary(files){
    ...
    if(String(cat.type || 'image').toLowerCase() === 'workflow'){
        setStatus('当前是工作流分组，请切换到图片、音频或视频分组上传');
        return;
    }
    const data = await uploadFilesToLibrary(list, activeCanvasAssetLibraryId, cat.id);
    if(data?.library){
        canvasAssetLibrary = data.library;
        renderCanvasAssetLibrary();
        setStatus(`已上传 ${list.length} 个资产到资产库`);
    }
}
...
function renderImageAssetManager(){
    ...
    assetManagerBody.innerHTML = `
        ...
        <div class="asset-manager-main">
            <div class="asset-manager-tools">
                <label class="${!cat ? 'disabled' : ''}"><i data-lucide="upload" class="w-4 h-4"></i><span>批量上传</span><input id="managerAssetUpload" type="file" multiple accept="image/*,video/*,audio/*,.mp3,.wav,.m4a,.aac,.ogg,.flac" ${!cat ? 'disabled' : ''}></label>
...
function currentCanvasPromptTemplateLibraryEditable(){
    // 系统库后端 readonly=false，也允许新增/编辑（走后端，与智能画布、资产库管理同步）。只按 readonly 判断。
    const lib = activeCanvasPromptLibrary();
...
function renderVideoBody(node){
    ...
    const promptInputs = ordered.filter(src => src.prompt && !src.refs?.length);
    sanitizeVideoNodeProviderModel(node);
    node.duration = Math.max(VOLC_VIDEO_DURATION_MIN, Math.min(VOLC_VIDEO_DURATION_MAX, Number(node.duration || 5)));
    node.count = clampVolcVideoCount(node.count || 1);
    if(!VOLC_VIDEO_ASPECTS.includes(node.aspectRatio)) node.aspectRatio = '16:9';
    if(!VOLC_VIDEO_RESOLUTIONS.includes(node.resolution)) node.resolution = '720p';
    const billing = volcVideoBillingEstimate(node);
    wrap.innerHTML = `
        <div class="prompt-list mb-3"></div>
        <div class="video-input-head">
            <div class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Media</div>
            <div class="video-billing-pill" data-yuan="${billing.yuan}" data-tokens="${billing.tokensTotal}" title="按火山方舟视频 Token 公式预估：宽 × 高 × 24fps × 秒数 / 1024 × 46元/百万token">
                <i data-lucide="coins" class="w-4 h-4"></i>
                <span>预计</span>
                <strong><small>¥</small><span data-video-billing-yuan>${formatYuan(billing.yuan)}</span></strong>
                <em><span data-video-billing-tokens>${billing.tokensTotal.toLocaleString()}</span><small>tokens</small></em>
            </div>
        </div>
        <div class="input-list video-img-list"></div>
        ${renderVideoTemporaryPublicLinks(node)}
        <div class="gen-settings">
            <div class="gen-settings-row">
            ...
            <div class="video-option-panel">
                <div class="video-option-row">
                    <span>${tr('canvas.videoAspect')}</span>
                    <div class="video-segment-options">${renderVideoOptionButtons(VOLC_VIDEO_ASPECTS, node.aspectRatio || '16:9', 'video-aspect-option')}</div>
                </div>
                <div class="video-option-row">
                    <span>${tr('canvas.videoResolution')}</span>
                    <div class="video-segment-options compact">${renderVideoOptionButtons(VOLC_VIDEO_RESOLUTIONS, node.resolution || '720p', 'video-resolution-option')}</div>
                </div>
            </div>
            <div class="gen-settings-row">
                <label class="field video-range-field">
                    <div class="video-range-title"><span>${tr('canvas.videoDuration')}</span><strong data-video-duration-value>${node.duration}秒</strong></div>
                    <input class="canvas-range video-duration" type="range" min="${VOLC_VIDEO_DURATION_MIN}" max="${VOLC_VIDEO_DURATION_MAX}" step="1" value="${node.duration}">
                </label>
            </div>
            <div class="gen-settings-row">
                <label class="field video-count-field">
                    <div class="video-range-title"><span>生成数量</span><strong data-video-count-value>${node.count}条</strong></div>
                    <input class="canvas-range video-count" type="range" min="${VOLC_VIDEO_COUNT_MIN}" max="${VOLC_VIDEO_COUNT_MAX}" step="1" value="${node.count}">
                </label>
            </div>
        ...
        <div class="gen-run-row">
            <button class="tool-btn video-test-btn" type="button"><i data-lucide="file-code-2" class="w-4 h-4"></i><span>测试</span></button>
    ...
    const countInput = wrap.querySelector('.video-count');
    providerSelect.value = node.apiProvider;
    durationSelect.value = String(node.duration || 5);
    countInput.value = String(node.count || 1);
    updateRangeFill(durationSelect);
    updateRangeFill(countInput);
    [providerSelect, modelSelect, durationSelect, countInput].forEach(input => {
        input.onmousedown = e => e.stopPropagation();
        input.onclick = e => e.stopPropagation();
    });
    providerSelect.onchange = e => {
    ...
    durationSelect.oninput = e => { e.stopPropagation(); node.duration = Math.max(VOLC_VIDEO_DURATION_MIN, Math.min(VOLC_VIDEO_DURATION_MAX, Number(e.target.value || 5))); updateRangeFill(durationSelect); refreshVideoBillingView(wrap, node); scheduleSave(); };
    durationSelect.onblur = e => { e.target.value = String(Math.max(VOLC_VIDEO_DURATION_MIN, Math.min(VOLC_VIDEO_DURATION_MAX, Number(node.duration || 5)))); };
    countInput.oninput = e => { e.stopPropagation(); node.count = clampVolcVideoCount(e.target.value); updateRangeFill(countInput); refreshVideoBillingView(wrap, node); scheduleSave(); };
    countInput.onblur = e => { e.target.value = String(clampVolcVideoCount(node.count || 1)); };
    wrap.querySelectorAll('[data-video-aspect-option]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            e.stopPropagation();
            node.aspectRatio = btn.dataset.videoAspectOption || '16:9';
            wrap.querySelectorAll('[data-video-aspect-option]').forEach(item => item.classList.toggle('active', item === btn));
            refreshVideoBillingView(wrap, node);
            scheduleSave();
        };
    });
    wrap.querySelectorAll('[data-video-resolution-option]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            e.stopPropagation();
            node.resolution = btn.dataset.videoResolutionOption || '720p';
            wrap.querySelectorAll('[data-video-resolution-option]').forEach(item => item.classList.toggle('active', item === btn));
            refreshVideoBillingView(wrap, node);
            scheduleSave();
        };
    });
    ...
    wrap.querySelectorAll('[data-video-temp-sh]').forEach(btn => {
        ...
        btn.onclick = async e => {
            ...
            try {
                ...
                showErrorModal(err.message || '上传到临时公网失败', '上传到临时公网');
            }
        };
    });
    wrap.querySelectorAll('[data-copy-temp-public]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = async e => {
            e.stopPropagation();
            await copyTextToClipboard(btn.dataset.copyTempPublic || '');
        };
    });
    wrap.querySelectorAll('[data-open-temp-public]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            e.stopPropagation();
            const url = btn.dataset.openTempPublic || '';
    ...
    renderPromptPreview(wrap.querySelector('.prompt-list'), promptInputs);
    wrap.querySelector('.gen-btn').onclick = e => { e.stopPropagation(); runCanvasGenerate(node.id); };
    wrap.querySelector('.video-test-btn').onclick = e => { e.stopPropagation(); testVideoNodeRequest(node.id); };
    bindCascadeButtons(wrap, node.id);
    return wrap;
}
...
function renderVideoImageInputs(list, node, imageInputs){
    ...
    bindResponsiveVideoInputGrid(list, imageInputs.length);
    refreshIcons();
}
function bindResponsiveVideoInputGrid(list, itemCount){
    if(!list) return;
    const update = () => {
        const count = Math.max(1, Number(itemCount || list.children.length || 1));
        const width = Math.max(0, list.clientWidth);
        if(!width) return;
        const gap = 8;
        const comfortableMin = width < 360 ? 68 : 72;
        const columnsThatFit = Math.max(1, Math.floor((width + gap) / (comfortableMin + gap)));
        const columns = Math.min(count, columnsThatFit);
        const cardWidth = Math.min(92, Math.max(comfortableMin, Math.floor((width - gap * (columns - 1)) / columns)));
        list.style.setProperty('--video-input-columns', String(columns));
        list.style.setProperty('--video-input-card-width', `${cardWidth}px`);
        list.classList.toggle('video-input-grid-compact', columns >= 7);
    };
    update();
    ...
    if(window.ResizeObserver){
        ...
        list._videoInputResizeObserver = observer;
    }
}
...
function outputForNode(node, dx=520){
...
function newOutputForNodeRun(node, dx=520){
    if(!node || !shouldCreateOutputForNode(node)) return null;
    const existing = outputNodesForSource(node.id);
    const offset = existing.length * 38;
    const out = {
        id:uid('out'),
        type:'output',
...
function generatorSources(gen){
    return connections.filter(c => c.to === gen.id).map(c => nodes.find(n => n.id === c.from)).filter(Boolean).map(n => {
        ...
        if(n.type === 'prompt') {
            const promptText = promptAssetDisplayText(n.text || '');
            return {id:n.id, type:'prompt', label:(promptText || '提示词').slice(0, 32), refs:promptNodeAssetRefs(n), prompt:promptText, rawPrompt:n.text || ''};
        }
        if(n.type === 'loop') {
        ...
        if(n.type === 'promptGroup') {
            ...
            const combined = prompts.join('\n\n');
            const promptText = promptAssetDisplayText(combined);
            const groupRefs = (n.items || [])
                .map(id => nodes.find(x => x.id === id))
                .filter(x => x?.type === 'prompt')
                .flatMap(promptNodeAssetRefs);
            return {id:n.id, type:'promptGroup', label:`提示词 ${prompts.length} 个`, refs:normalizePromptAssetRefs([...groupRefs, ...promptAssetMentionRefs(combined)]), prompt:promptText, rawPrompt:combined};
        }
...
function videoPromptWithAssetIndexes(prompt, mediaRefs){
    const refs = Array.isArray(mediaRefs) ? mediaRefs : [];
    const indexes = {image:new Map(), video:new Map(), audio:new Map()};
    const names = {image:new Map(), video:new Map(), audio:new Map()};
    const counts = {image:0, video:0, audio:0};
    refs.forEach(ref => {
        const kind = mediaKindForRef(ref);
        if(!indexes[kind] || !ref?.url || indexes[kind].has(ref.url)) return;
        counts[kind] += 1;
        indexes[kind].set(ref.url, counts[kind]);
        const cleanName = String(ref.name || ref.title || ref.label || outputImageName(ref.url) || '').trim();
        if(cleanName && !names[kind].has(cleanName)) names[kind].set(cleanName, counts[kind]);
    });
    const labels = {image:'图片', video:'视频', audio:'音频'};
    let text = String(prompt || '').replace(/@\[([^\]]{1,120})\]\(([^)\s]+)\)/g, (match, name, url) => {
        const cleanUrl = String(url || '').trim();
        const kind = mediaKindForRef({url:cleanUrl});
        const index = indexes[kind]?.get(cleanUrl);
        const cleanName = String(name || outputImageName(cleanUrl) || '素材').trim();
        if(!index || !labels[kind]) return cleanName ? `@${cleanName}` : '';
        return `${labels[kind]}${index}（${cleanName}）`;
    });
    const replaceAudioContextMention = (name, index) => {
        const escaped = String(name).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        text = text.replace(
            new RegExp(`(声音参考|音频参考|参考声音|配音参考)\\s*@${escaped}`, 'g'),
            `$1${labels.audio}${index}（${name}）`
        );
    };
    Array.from(names.audio.entries())
        .sort((a, b) => String(b[0]).length - String(a[0]).length)
        .forEach(([name, index]) => replaceAudioContextMention(name, index));
    const replacePlainMention = (kind, name, index) => {
        const escaped = String(name).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        text = text.replace(new RegExp(`@${escaped}`, 'g'), `${labels[kind]}${index}（${name}）`);
    };
    ['image','video','audio'].forEach(kind => {
        Array.from(names[kind].entries())
            .sort((a, b) => String(b[0]).length - String(a[0]).length)
            .forEach(([name, index]) => {
                if(kind === 'audio' && (names.image.has(name) || names.video.has(name))) return;
                replacePlainMention(kind, name, index);
            });
    });
    return text;
}
function uniqueMediaRefs(refs=[]){
    const seen = new Set();
...
function videoNodeRequestPayload(node){
    const sources = orderedSources(node, generatorSources(node));
    const sourcePrompts = sources.map(s => s.prompt).filter(Boolean);
    const rawPrompt = sourcePrompts.join('\n\n');
    const richPrompt = sources
        .map(s => s.rawPrompt || s.sourceText || s.prompt)
        .filter(Boolean)
        .join('\n\n') || rawPrompt;
    const allRefs = sources.flatMap(s => s.refs || []);
    const mediaRefs = uniqueMediaRefs(applyUploadedUrlToRefs((allRefs || []).filter(ref => ['image','video','audio'].includes(mediaKindForRef(ref))), node));
    const refs = imageRefsOnly(mediaRefs);
    const videoRefs = videoRefsOnly(mediaRefs);
    const audioRefs = audioRefsOnly(mediaRefs);
    const prompt = videoPromptWithAssetIndexes(rawPrompt, mediaRefs);
    const hasInlineAssetRefs = promptAssetMentionRefs(rawPrompt).length > 0;
    if(node.useFrameRoles && refs[0]) refs[0] = {...refs[0], role:'first_frame'};
    if(node.useFrameRoles && refs[1]) refs[1] = {...refs[1], role:'last_frame'};
    const payload = {
        prompt,
        provider_id:resolveVideoProviderId(node.apiProvider || 'volcengine'),
        model:node.model || '',
        duration:Math.max(VOLC_VIDEO_DURATION_MIN, Math.min(VOLC_VIDEO_DURATION_MAX, Number(node.duration || 5))),
        count:clampVolcVideoCount(node.count || 1),
        aspect_ratio:VOLC_VIDEO_ASPECTS.includes(node.aspectRatio) ? node.aspectRatio : '16:9',
        resolution:VOLC_VIDEO_RESOLUTIONS.includes(node.resolution) ? node.resolution : '720p',
        images:refs,
        ...
        trusted_asset:true,
        multimodal:Boolean(node.multimodal || (hasInlineAssetRefs && refs.length) || (!node.useFrameRoles && refs.length > 1))
    };
    return {prompt, richPrompt, refs:mediaRefs, payload};
}
const videoReferenceDurationCache = new Map();
function mediaReferenceDuration(url, kind){
    const cleanUrl = String(url || '').trim();
    if(!cleanUrl || !['video','audio'].includes(kind)) return Promise.resolve(null);
    if(videoReferenceDurationCache.has(cleanUrl)) return videoReferenceDurationCache.get(cleanUrl);
    ...
    videoReferenceDurationCache.set(cleanUrl, promise);
    return promise;
}
async function validateVideoNodeReferences(payload, refs=[]){
    const images = uniqueMediaRefs((refs || []).filter(ref => mediaKindForRef(ref) === 'image'));
    const videos = uniqueMediaRefs((refs || []).filter(ref => mediaKindForRef(ref) === 'video'));
    const audios = uniqueMediaRefs((refs || []).filter(ref => mediaKindForRef(ref) === 'audio'));
    const errors = [];
    if(images.length > 9) errors.push(`参考图片最多 9 张，当前 ${images.length} 张`);
    if(videos.length > 3) errors.push(`参考视频最多 3 个，当前 ${videos.length} 个`);
    if(audios.length > 3) errors.push(`参考音频最多 3 个，当前 ${audios.length} 个`);
    for(const [kind, list, label] of [['video', videos, '视频'], ['audio', audios, '音频']]){
        const durations = await Promise.all(list.map(ref => mediaReferenceDuration(ref.url, kind)));
        let total = 0;
        durations.forEach((duration, index) => {
            if(duration == null) return;
            total += duration;
            if(duration < 2 || duration > 15) errors.push(`参考${label}「${list[index].name || index + 1}」时长为 ${duration.toFixed(1)} 秒，单个必须在 2–15 秒之间`);
        });
        if(total > 15.05) errors.push(`所有参考${label}总时长最多 15 秒，当前约 ${total.toFixed(1)} 秒`);
    }
    if(errors.length){
...
function videoTestPreviewText(result, payload, refs, prompt){
    const safeRefs = Array.isArray(refs) ? refs : [];
    const billing = volcVideoBillingEstimate({
        duration:payload?.duration,
        aspect_ratio:payload?.aspect_ratio,
        resolution:payload?.resolution,
        count:payload?.count || 1
    });
    const countByKind = kind => safeRefs.filter(ref => mediaKindForRef(ref) === kind).length;
    const requestJson = result?.json || {};
    const content = Array.isArray(requestJson.content) ? requestJson.content : [];
    const requestTextItems = content.filter(item => item?.type === 'text').length;
    const requestImages = content.filter(item => item?.type === 'image_url').length;
    const requestVideos = content.filter(item => item?.type === 'video_url').length;
    const requestAudios = content.filter(item => String(item?.type || '').includes('audio')).length;
    const refLines = safeRefs.length
        ? safeRefs.map((ref, index) => {
            const name = ref.name || ref.title || ref.label || `素材 ${index + 1}`;
            const role = ref.role ? ` / ${ref.role}` : '';
            return `${index + 1}. ${mediaKindForRef(ref)}${role}：${name}`;
        }).join('\n')
        : '无';
    const lifecycle = result?.task_lifecycle || {};
    const lifecycleOrder = ['create','query','list','cancel_or_delete'];
    const lifecycleNames = {
        create:'1. 创建视频任务',
        query:'2. 查询任务结果',
        list:'3. 查询任务列表',
        cancel_or_delete:'4. 取消或删除任务'
    };
    const lifecycleLines = lifecycleOrder
        .filter(key => lifecycle[key])
        .map(key => {
            const item = lifecycle[key] || {};
            const details = [item.body, item.until, item.filters, item.rule].filter(Boolean).join(' ');
            return `${lifecycleNames[key] || key}\n${item.method || 'GET'} ${item.url || ''}${details ? `\n${details}` : ''}`;
        }).join('\n\n');
    const promptPreview = String(prompt || '').replace(/\s+/g, ' ').trim();
    return [
        '火山方舟 Seedance 测试请求预览（不会发送，不消耗 token）',
        '',
        '【摘要】',
        `接口：${result?.method || 'POST'} ${result?.url || ''}`,
        `模型：${payload?.model || '<请在 API 设置中拉取或手动填写视频模型>'}`,
        `参数：${payload?.duration || '-'} 秒 / ${payload?.aspect_ratio || '-'} / ${payload?.resolution || '-'}`,
        `计费预估：${formatVideoBillingShort(billing)}（${billing.width}×${billing.height} / ${billing.fps}fps / ${billing.count}条）`,
        `音频：${payload?.generate_audio ? '生成音频' : '不生成音频'}`,
        `素材来源：图片 ${countByKind('image')} / 音频 ${countByKind('audio')} / 视频 ${countByKind('video')}`,
        `请求内容：文本 ${requestTextItems} / 图片 ${requestImages} / 音频 ${requestAudios} / 视频 ${requestVideos}`,
        '',
        '【火山任务流程】',
        lifecycleLines || '无',
        result?.task_lifecycle?.success_output ? `\n成功输出：${result.task_lifecycle.success_output}` : '',
        '',
        '【提示词】',
        promptPreview || '无',
        '',
...
testVideoNodeRequest = async function(nodeId){
    const node = nodes.find(n => n.id === nodeId);
    if(!node) return;
    const {prompt, richPrompt, refs, payload} = videoNodeRequestPayload(node);
    if(!prompt){ showErrorModal(tr('canvas.videoNeedsPrompt'), '测试请求'); return; }
    if(!await validateVideoNodeReferences(payload, refs)) return;
    try {
        const result = await fetch('/api/canvas-video/test', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify(payload)
        }).then(async r => { if(!r.ok) throw new Error(await responseErrorMessage(r, '测试请求整理失败')); return r.json(); });
        const text = videoTestPreviewText(result, payload, refs, prompt);
        const p = {x:(Number(node.x) || 0) + 440, y:Number(node.y) || 0};
        addNode({id:uid('prompt'), type:'prompt', x:p.x, y:p.y, w:520, h:480, text});
        scheduleSave();
    } catch(err) {
        showErrorModal(err.message || '测试请求整理失败', '测试请求');
    }
};
async function runVideoNode(nodeId, opts={}){
    ...
    if(!payload.model){ alert('请先到 API 设置填写 API Key，并拉取或手动添加视频模型。'); return; }
    if(!await validateVideoNodeReferences(payload, refs)) return;
    let out = newOutputForNodeRun(node, 520);
    const pendingId = uid('p');
    const run = runSnapshot(node, prompt, refs);
    run.billing = videoBillingFromPayload(payload, payload.count || 1);
    run.richPrompt = richPrompt || prompt;
    ...
    try {
        const requestCount = clampVolcVideoCount(payload.count || 1);
        const submitPayload = {...payload};
        delete submitPayload.count;
        const results = [];
        ...
        const meta = collectRunMeta(out, pendingId);
        if(out) out._pending = (out._pending || []).filter(p => p.id !== pendingId);
        const singleBilling = videoSingleBillingFromEstimate(run.billing);
        const outputUrls = results.flatMap(result => resultMediaUrls(result)).map(item => {
            const url = outputUrlValue(item);
            return item && typeof item === 'object'
                ? {...item, url, kind:item.kind || 'video', aspectRatio:item.aspectRatio || item.aspect_ratio || payload.aspect_ratio || node.aspectRatio || '16:9', billing:item.billing || singleBilling}
                : {url, kind:'video', aspectRatio:payload.aspect_ratio || node.aspectRatio || '16:9', billing:singleBilling};
        }).filter(item => item.url);
        if(!outputUrls.length) throw new Error(tr('canvas.videoFailed'));
        const requestMetas = results.map(result => requestMetaFromResult(result)).filter(Boolean);
        run.request = requestMetas.length === 1 ? requestMetas[0] : {
            task_id: requestMetas.map(item => item.task_id).filter(Boolean).join(', '),
            request_id: requestMetas.map(item => item.request_id).filter(Boolean).join(', '),
            count:requestCount
        };
        const outputMetas = outputUrls.map(() => ({...meta, kind:'video', aspectRatio:payload.aspect_ratio || node.aspectRatio || '16:9', billing:singleBilling}));
        appendOutputImages(out, outputUrls, refs[0], outputMetas);
        mergeGeneratedOutputs(node, outputUrls, Boolean(opts.cascade));
...
function runSnapshot(node, prompt, refs=[]){
    ...
    return {
        ...
        billing: node?.type === 'video' ? volcVideoBillingEstimate(node) : null,
    };
}
function cleanLogSnapshotNode(node){
    return JSON.parse(JSON.stringify(serializableCanvasNode(node || {})));
}
function collectGenerationWorkflowSnapshot(run){
    const focusId = String(run?.node?.id || '');
    if(!focusId) return null;
    const byId = new Map((nodes || []).map(node => [node.id, node]));
    if(!byId.has(focusId)) return null;
    const included = new Set();
    const includeNode = id => {
        if(!id || included.has(id) || !byId.has(id)) return;
        included.add(id);
        const node = byId.get(id);
        if((node.type === 'group' || node.type === 'promptGroup') && Array.isArray(node.items)){
            node.items.forEach(includeNode);
        }
    };
    const walkUpstream = id => {
        includeNode(id);
        (connections || []).filter(conn => conn.to === id).forEach(conn => {
            includeNode(conn.from);
            walkUpstream(conn.from);
        });
    };
    walkUpstream(focusId);
    (connections || []).filter(conn => conn.from === focusId).forEach(conn => {
        const to = byId.get(conn.to);
        if(to?.type === 'output') includeNode(conn.to);
    });
    const snapshotNodes = [...included]
        .map(id => byId.get(id))
        .filter(Boolean)
        .map(cleanLogSnapshotNode);
    const snapshotConnections = (connections || [])
        .filter(conn => included.has(conn.from) && included.has(conn.to))
...
function workflowSnapshotBounds(snapshotNodes=[]){
    const rects = snapshotNodes.map(node => {
        const size = defaultNodeSize(node.type);
        const w = Number(node.w || size.w || 260) || 260;
        const h = Number(node.h || size.h || 220) || 220;
        return {x:Number(node.x || 0), y:Number(node.y || 0), w, h};
    }).filter(Boolean);
    if(!rects.length) return {x:0, y:0, w:360, h:260};
...
function canvasRectsOverlap(a, b, pad=0){
    return a.x < b.x + b.w + pad
        && a.x + a.w + pad > b.x
        && a.y < b.y + b.h + pad
        && a.y + a.h + pad > b.y;
}
function isWorkflowPlacementClear(x, y, w, h, pad=96){
    const candidate = {x, y, w, h};
    return !(nodes || []).some(node => canvasRectsOverlap(candidate, estimatedNodeRect(node), pad));
}
function allCanvasContentBounds(){
    const rects = (nodes || []).map(estimatedNodeRect);
    if(!rects.length) return currentWorldViewRect();
    const x1 = Math.min(...rects.map(r => r.x));
    const y1 = Math.min(...rects.map(r => r.y));
    const x2 = Math.max(...rects.map(r => r.x + r.w));
    const y2 = Math.max(...rects.map(r => r.y + r.h));
...
function findAvailableWorkflowPosition(w, h){
    const width = Math.max(260, Number(w || 0));
    const height = Math.max(180, Number(h || 0));
    const view = currentWorldViewRect();
    const centerX = view.x + view.w / 2;
    const centerY = view.y + view.h / 2;
    const stepX = Math.max(420, Math.min(1100, width + 220));
    const stepY = Math.max(320, Math.min(900, height + 220));
    const candidates = [];
    const push = (x, y) => candidates.push({x, y, d:Math.hypot(x + width / 2 - centerX, y + height / 2 - centerY)});
    push(centerX - width / 2, centerY - height / 2);
    for(let ring = 1; ring <= 9; ring += 1){
        for(let ix = -ring; ix <= ring; ix += 1){
            for(let iy = -ring; iy <= ring; iy += 1){
                ...
                push(centerX - width / 2 + ix * stepX, centerY - height / 2 + iy * stepY);
            }
        }
    }
    candidates.sort((a, b) => a.d - b.d);
    const found = candidates.find(pos => isWorkflowPlacementClear(pos.x, pos.y, width, height));
    if(found) return {x:found.x, y:found.y};
    const content = allCanvasContentBounds();
    return {x:content.x + content.w + 260, y:Math.min(view.y + 120, content.y)};
}
function fitGroupToRestoredNodes(groupId, itemIds=[]){
    const group = nodes.find(node => node.id === groupId && node.type === 'group');
    if(!group || !itemIds.length) return false;
    const box = nodeBounds(itemIds);
...
function addGenerationLog({run, outputs=[], runMs=0, error=''}) {
    ...
    const entry = {
        ...
        request:run?.request || {},
        prompt:run?.prompt || '',
        richPrompt:run?.richPrompt || '',
        outputs:(outputs || []).filter(Boolean),
        refs:run?.refs || [],
        billing:run?.billing || null,
        workflowSnapshot:collectGenerationWorkflowSnapshot(run),
        videoSettings:run?.nodeType === 'video' ? {
            apiProvider:run?.node?.apiProvider || 'volcengine',
            model:run?.node?.model || '',
            duration:run?.node?.duration || run?.billing?.duration || 5,
            aspectRatio:run?.node?.aspectRatio || run?.billing?.aspectRatio || '16:9',
            resolution:run?.node?.resolution || run?.billing?.resolution || '720p',
            generateAudio:run?.node?.generateAudio !== false,
            watermark:Boolean(run?.node?.watermark),
            multimodal:Boolean(run?.node?.multimodal),
            useFrameRoles:Boolean(run?.node?.useFrameRoles)
        } : null,
...
function restoreWorkflowSnapshotFromLog(logId){
    if(!ensureCanvas()) return;
    const log = (canvas.logs || []).find(item => item.id === logId);
    if(!log) return;
    const snapshot = log.workflowSnapshot;
    if(!snapshot?.nodes?.length){
        if(isVideoGenerationLog(log)) restoreVideoHistoryToCanvas(logId);
        return;
    }
    const sourceNodes = clonePlain(snapshot.nodes, []).filter(Boolean);
    const sourceConnections = clonePlain(snapshot.connections || [], []).filter(Boolean);
    if(!sourceNodes.length) return;
    const bounds = workflowSnapshotBounds(sourceNodes);
    const target = findAvailableWorkflowPosition(bounds.w + 68, bounds.h + 108);
    const dx = (target.x + 34) - bounds.x;
    const dy = (target.y + 64) - bounds.y;
    const idMap = new Map();
    sourceNodes.forEach(node => idMap.set(node.id, uid(String(node.type || 'node').replace(/[^a-z0-9_-]/ig, '') || 'node')));
    const restoredNodes = sourceNodes.map(source => {
        const node = {...source};
        const oldId = source.id;
        node.id = idMap.get(oldId);
        node.x = Number(source.x || 0) + dx;
        node.y = Number(source.y || 0) + dy;
        delete node.running;
        delete node.runStatus;
        delete node.runError;
        delete node._pending;
        delete node._ltxEditor;
        if(Array.isArray(node.items)) node.items = node.items.map(id => idMap.get(id)).filter(Boolean);
        if(Array.isArray(node.inputs)) node.inputs = node.inputs.map(id => idMap.get(id) || id);
        return node;
    });
    const restoredIds = restoredNodes.map(node => node.id);
    const restoredConnections = sourceConnections
        .filter(conn => idMap.has(conn.from) && idMap.has(conn.to))
        .map(conn => ({...conn, id:uid('c'), from:idMap.get(conn.from), to:idMap.get(conn.to)}));
    const box = workflowSnapshotBounds(restoredNodes);
    const groupNode = {
        id:uid('grp'),
        type:'group',
        name:'日志恢复',
    ...
    pushUndo();
    nodes.push(groupNode, ...restoredNodes);
    ...
    render();
    requestAnimationFrame(() => {
        if(fitGroupToRestoredNodes(groupNode.id, restoredIds)){
            render();
            scheduleSave();
        }
        focusViewportOnWorldRect(estimatedNodeRect(groupNode));
    });
    scheduleSave();
    closeCanvasLog();
}
function renderCanvasLog(){
    ...
    list.innerHTML = logs.length ? logs.map(log => {
        ...
        const billingText = log.billing?.yuan ? `预计 ¥${formatYuan(log.billing.yuan)}` : '';
        const subParts = [
            date,
        ...
        return `<div class="log-item ${log.status === 'failed' ? 'failed' : ''}">
            <div class="log-main">
                <div class="log-meta">
                    ...
                    ${billingText ? `<span class="log-chip">${escapeHtml(billingText)}</span>` : ''}
                </div>
                ...
                <div class="log-actions">
                    <button type="button" class="log-action-btn" data-log-restore="${escapeAttr(log.id)}"><i data-lucide="rotate-ccw"></i><span>恢复节点</span></button>
                    ${log.prompt ? `<button type="button" class="log-action-btn secondary" data-log-copy-prompt="${escapeAttr(log.id)}"><i data-lucide="copy"></i><span>复制提示词</span></button>` : ''}
                </div>
            </div>
    ...
    list.querySelectorAll('[data-log-restore]').forEach(btn => {
        btn.onclick = e => {
            e.stopPropagation();
            restoreWorkflowSnapshotFromLog(btn.dataset.logRestore);
        };
    });
    list.querySelectorAll('[data-log-copy-prompt]').forEach(btn => {
        btn.onclick = e => {
            e.stopPropagation();
            const log = (canvas?.logs || []).find(item => item.id === btn.dataset.logCopyPrompt);
            if(log?.prompt) navigator.clipboard?.writeText(log.prompt).catch(() => {});
            const span = btn.querySelector('span');
            if(span){
                const old = span.textContent;
                span.textContent = tr('canvas.copied');
                setTimeout(() => { span.textContent = old; }, 900);
            }
        };
    });
...
window.restoreWorkflowSnapshotFromLog = restoreWorkflowSnapshotFromLog;
...
function mergeGeneratedOutputs(node, outputs, append=false){
    if(!node) return;
    // generatedOutputs feeds downstream nodes; the result card itself is populated by
    // the run that created it. Broadcasting here would copy a later run into every
    // historical output node connected to the generator.
    const keepGeneratedMedia = ['rh','ltxDirector','video'].includes(node.type);
    const clean = (outputs || []).map(item => {
        ...
        return {
            ...
            url,
            kind,
            aspectRatio:source.aspectRatio || source.aspect_ratio || source.ratio || node.aspectRatio || '',
...
function renderOutputMedia(item, useGridLayout=false){
    ...
    if(kind === 'video'){
        const aspect = outputAspectFromMeta(meta);
        const videoStyle = gridStyle || (aspect ? ` style="--output-aspect:${aspect}"` : '');
        return `<div class="output-img-wrap output-video-wrap" data-output-url="${safe}"${videoStyle}><div class="output-video-stage">${canvasVideoFallbackHtml(url, 'controls data-output-video-fallback=&quot;1&quot;')}${timePill}<button class="output-del" title="${tr('common.delete')}">×</button></div>${outputBillingHtml(meta)}<button class="output-download output-video-download" type="button" title="${tr('canvas.download')}" aria-label="${tr('canvas.download')}"><i data-lucide="download"></i><span>下载视频</span></button></div>`;
    }
    if(kind === 'audio'){
        return `<div class="output-img-wrap output-audio-wrap" data-output-url="${safe}"${gridStyle}><div class="output-audio-card"><i data-lucide="file-audio" class="w-7 h-7"></i><span>${escapeHtml(outputImageName(url))}</span><audio src="${safe}" data-url="${safe}" controls preload="metadata"></audio></div>${timePill}<button class="output-download" type="button" title="${tr('canvas.download')}" aria-label="${tr('canvas.download')}"><i data-lucide="download"></i></button><button class="output-del" title="${tr('common.delete')}">×</button></div>`;
    }
...
function appendOutputImages(out, images, compareRef, metas=[], layout=null){
    ...
    out.images = [...(out.images || []), ...list.map((url, i) => {
        ...
        if(source.aspectRatio || source.aspect_ratio || source.ratio) item.aspectRatio = source.aspectRatio || source.aspect_ratio || source.ratio;
        if(meta.kind) item.kind = meta.kind;
        if(meta.aspectRatio || meta.aspect_ratio || meta.ratio) item.aspectRatio = meta.aspectRatio || meta.aspect_ratio || meta.ratio;
...
canvasAssetUploadBtn?.addEventListener('click', () => {
    if(canvasAssetUploadBtn.disabled) return;
    canvasAssetUploadInput?.click();
});
canvasAssetUploadInput?.addEventListener('change', async () => {
    try {
        await uploadFilesToActiveCanvasAssetLibrary(canvasAssetUploadInput.files);
    } catch(err) {
        showErrorModal(err.message || '上传资产失败', '上传资产失败');
    } finally {
        if(canvasAssetUploadInput) canvasAssetUploadInput.value = '';
    }
});
canvasAssetAddCategoryBtn?.addEventListener('click', async () => {
    if(canvasAssetLibraryIsLocal()){ setStatus('本地素材请在资产库管理中管理文件夹'); return; }
    const name = window.prompt('新分组名称', '新分组');
    if(!String(name || '').trim()) return;
    const data = await fetch('/api/asset-library/categories', {
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({library_id:activeCanvasAssetLibraryId, name:String(name).trim(), type:String(activeCanvasAssetCategory()?.type || 'image').toLowerCase()})
    }).then(r => r.json());
    canvasAssetLibrary = data.library || canvasAssetLibrary;
...
canvasAssetDropZone?.addEventListener('drop', async event => {
    ...
    try {
        ...
        if(payload.type === 'files'){
            await uploadFilesToActiveCanvasAssetLibrary(payload.files);
...
assetManagerModal?.addEventListener('click', async event => {
    ...
    if(event.target.closest?.('[data-manager-asset-cat-new]')){
        ...
        if(!String(name || '').trim()) return;
        const data = await fetch('/api/asset-library/categories', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({library_id:activeCanvasAssetLibraryId, name, type:String(activeCanvasMediaCategory()?.type || 'image').toLowerCase()})}).then(r => r.json());
        canvasAssetLibrary = data.library || canvasAssetLibrary;
        activeCanvasAssetCategoryId = data.category?.id || activeCanvasAssetCategoryId;
        renderAssetManager(); renderCanvasAssetLibrary(); return;
    }
...
function selectedGroupableNodes(){
    return [...selected]
        .map(id => nodes.find(n => n.id === id))
...
function promptCanvasGroupName(current=''){
    const fallback = current || nextGroupName();
    const name = window.prompt('给这个组命名', fallback);
    if(name === null) return null;
    return String(name || '').trim().slice(0, 40) || fallback;
}
function renameGroupNode(groupId){
    const group = nodes.find(n => n.id === groupId && n.type === 'group');
    if(!group) return;
    const name = promptCanvasGroupName(group.name || '');
    if(name === null) return;
    pushUndo();
    group.name = name;
    render();
...
function ungroupNode(groupId){
    const group = nodes.find(n => n.id === groupId && n.type === 'group');
    if(!group) return;
    const childIds = new Set((group.items || []).filter(id => nodes.some(n => n.id === id)));
    pushUndo();
    const outgoing = connections.filter(c => c.from === group.id).map(c => c.to);
    childIds.forEach(childId => {
        outgoing.forEach(targetId => {
            if(canConnect(childId, targetId) && !connections.some(c => c.from === childId && c.to === targetId)){
                connections.push({id:uid('c'), from:childId, to:targetId});
            }
        });
    });
    connections = connections.filter(c => c.from !== group.id && c.to !== group.id);
    nodes = nodes.filter(n => n.id !== group.id);
    nodes.filter(n => n.type === 'group' && Array.isArray(n.items)).forEach(n => {
        n.items = n.items.filter(id => id !== group.id);
    });
    selected.clear();
...
function groupSelectedNodes(){
    if(!ensureCanvas()) return;
    const targets = selectedGroupableNodes();
    if(!targets.length) return;
    const name = nextGroupName();
    const box = nodeBounds(targets.map(n => n.id));
    const group = {id:uid('grp'), type:'group', name, x:box.x - 24, y:box.y - 58, w:box.w + 48, h:box.h + 90, items:targets.map(n => n.id)};
    pushUndo();
    nodes.push(group);
    selected.clear();
    selected.add(group.id);
...
function startSelection(e, options={}){
    e.preventDefault();
    e.stopPropagation();
    if(document.activeElement && document.activeElement !== document.body) document.activeElement.blur();
    selectDrag = {sx:e.clientX, sy:e.clientY, x:e.clientX, y:e.clientY, button:e.button, context:Boolean(options.context), moved:false};
    document.body.classList.add('canvas-selecting');
    selectionBox.style.display = 'block';
    updateSelectionBox(e.clientX, e.clientY);
    const moveHandler = e2 => updateSelectionBox(e2.clientX, e2.clientY);
    const upHandler = () => finishSelection();
    if(options.pointer){
...
function updateSelectionBox(x, y){
    ...
    if(Math.abs(x - selectDrag.sx) > 4 || Math.abs(y - selectDrag.sy) > 4) selectDrag.moved = true;
...
function finishSelection(){
    if(!selectDrag) return;
    const wasContextSelection = selectDrag.context;
    ...
    if(wasContextSelection && wasMoved) suppressNextContextMenu = true;
    selectDrag = null;
...
function downloadUrl(url, filename='download'){
    ...
    if(window.showSaveFilePicker && !raw.startsWith('data:') && !raw.startsWith('blob:')){
        return fetch(href)
            .then(async res => {
                if(!res.ok) throw new Error(await responseErrorMessage(res, '下载失败'));
                const blob = await res.blob();
                const handle = await window.showSaveFilePicker({
                    suggestedName: filename || outputDownloadName(raw),
                    types:[{description:'视频文件', accept:{[blob.type || 'video/mp4']:[extensionFromNameOrUrl(filename, raw) || '.mp4']}}]
                });
...
function updateGroupMembership(movedNodes){
    ...
    const children = movedNodes.filter(n => n && n.type !== 'group' && n.type !== 'promptGroup');
...
function refreshSelectionVisuals(){
    nodesEl.querySelectorAll('.node').forEach(el => {
        ...
        el.classList.toggle('node-front', canFloatFront && frontNodeId === el.dataset.id);
    });
    renderLinks();
    renderSelectionHub();
    if(workflowTransferModal?.classList.contains('open')) updateWorkflowTransferMeta();
...
function isZoomPreviewIgnoredTarget(target){
    return !!target?.closest?.('#createMenu, #linkCreateMenu, #nodeInputMenu, #nodeOutputMenu, #imageNodeMenu, .minimap, #canvasAssetPanel, #assetManagerModal, #workflowTransferModal, #logModal, #promptTemplateModal, #promptAssetMentionModal, #imageEditModal, #outputLightbox');
}
...
board.onmousedown = e => {
    ...
    if(e.button === 2 && (e.target === board || e.target === world || e.target === nodesEl || e.target === linksEl)){
        closeCreateMenu();
        startSelection(e, {context:true});
        return;
    }
    if(e.button !== 0) return;
    if(startKnifeDrag(e)) return;
...
board.addEventListener('pointerdown', e => {
    ...
    startSelection(e, {context:true, pointer:true});
}, true);
...
window.addEventListener('keydown', e => {
    ...
    if(e.key === 'Escape' && document.getElementById('promptAssetMentionModal')) { closePromptAssetMentionModal(); return; }
    ...
    if((e.ctrlKey || e.metaKey) && key === 'g') { e.preventDefault(); groupSelectedNodes(); }
...
window.onload = async () => {
    ...
    videoHistoryToggle?.addEventListener('click', e => {
        e.preventDefault();
        e.stopPropagation();
        toggleVideoHistoryPanel();
    });
    videoHistoryCloseBtn?.addEventListener('click', e => {
        e.preventDefault();
        e.stopPropagation();
        toggleVideoHistoryPanel(false);
    });
