...
function apiNotice(message, kind='info'){
    const text = String(message || '').trim();
    if(!text) return;
    setStatus(text);
    showVerifyResult(`<div class="api-inline-notice ${escapeAttr(kind)}">${escapeHtml(text)}</div>`);
}
...
function isProviderTemporarilyHidden(item){
    if(ADAMI_VOLCENGINE_ONLY && (providers || []).some(provider => provider.id === 'volcengine')){
        return item?.id !== 'volcengine';
    }
    return false;
}
...
function isNewUserProvider(item){
    ...
    if(item.id === 'volcengine') return !item.has_key || !(item.video_models || []).length;
...
function renderProviderOnboarding(item){
    ...
    if(item.id === 'volcengine'){
        const hasKey = Boolean(item.has_key);
        const hasVideoModel = Boolean((item.video_models || []).length);
        providerOnboardingCard.innerHTML = `
            <div class="onboarding-head">
                <div>
                    <div class="onboarding-title">火山方舟接入流程</div>
                    <div class="onboarding-desc">按顺序完成后，视频生成节点就能选择真实 Seedance 模型；测试按钮仍然不会消耗 token。</div>
                </div>
                <span class="onboarding-badge">Seedance</span>
            </div>
            <div class="adami-api-flow">
                <div class="adami-api-step ${hasKey ? 'done' : ''}">
                    <span class="adami-api-step-num">1</span>
                    <div>
                        <strong>填写方舟 API Key</strong>
                        <span>把火山方舟控制台里的 API Key 粘贴到下面的 Key 输入框。</span>
                    </div>
                </div>
                <div class="adami-api-step ${hasKey ? 'done' : ''}">
                    <span class="adami-api-step-num">2</span>
                    <div>
                        <strong>保存 API 设置</strong>
                        <span>Key 会保存到本地后端配置里，页面只显示脱敏状态。</span>
                    </div>
                    <button class="onboarding-save-btn" type="button" onclick="saveKeyOnly()"><i data-lucide="check" class="w-3.5 h-3.5"></i><span>保存 Key</span></button>
                </div>
                <div class="adami-api-step ${hasVideoModel ? 'done' : ''}">
                    <span class="adami-api-step-num">3</span>
                    <div>
                        <strong>拉取或添加视频模型</strong>
                        <span>建议先点“拉取模型”，拉不到时可以手动添加 Seedance 模型名称。</span>
                    </div>
                    <div class="adami-api-step-actions">
                        <button class="onboarding-key-btn" type="button" onclick="fetchModels()"><i data-lucide="download-cloud" class="w-3.5 h-3.5"></i><span>拉取模型</span></button>
                        <button class="onboarding-key-btn" type="button" onclick="addModel('video')"><i data-lucide="plus" class="w-3.5 h-3.5"></i><span>手动添加</span></button>
                    </div>
                </div>
                <div class="adami-api-step ${hasKey && hasVideoModel ? 'done' : ''}">
                    <span class="adami-api-step-num">4</span>
                    <div>
                        <strong>回到画布测试</strong>
                        <span>视频节点点击“测试”，会整理请求预览，不会真的发送给火山。</span>
                    </div>
                </div>
            </div>
        `;
...
function openRecommendApi(){
    if(ADAMI_VOLCENGINE_ONLY){
        recommendInlineOpen = false;
        selectedId = 'volcengine';
        syncRecommendView();
        renderEditor();
        apiNotice('Adami-Canvas 当前只保留火山方舟配置。', 'ok');
        return;
    }
...
async function fetchModels(){
    ...
    const isJimeng = item.id === 'jimeng' || (protocolInput?.value || '') === 'jimeng';
    if(!baseUrl && !isJimeng){ alert('请先填写请求地址'); return; }
    if(!apiKey && !item.has_key && !isJimeng){
        alert('请先填写并保存 API Key，再拉取模型。');
        setStatus('请先填写并保存 API Key');
        return;
    }
...
const adamiOriginalFetchModels = fetchModels;
fetchModels = async function(){
    ...
    const isJimeng = item.id === 'jimeng' || (protocolInput?.value || '') === 'jimeng';
    if(!baseUrl && !isJimeng){
        apiNotice('请先填写请求地址。', 'warn');
        return;
    }
    if(!apiKey && !item.has_key && !isJimeng){
        apiNotice('请先填写并保存 API Key，再拉取模型。', 'warn');
        return;
    }
    try {
        await adamiOriginalFetchModels();
    } catch(e) {
        apiNotice(`拉取失败：${e.message || e}`, 'warn');
    }
};
const adamiOriginalOpenModelPicker = openModelPicker;
openModelPicker = function(){
    if(!lastFetchedAll.length){
        apiNotice('还没有拉取到模型，请先填写 Key 并点击“拉取模型”。', 'warn');
        return;
    }
    return adamiOriginalOpenModelPicker();
};
...
const adamiOriginalSaveKeyOnly = saveKeyOnly;
saveKeyOnly = async function(){
    const item = provider();
    if(!item) return;
    if(!String(keyInput?.value || '').trim()){
        apiNotice('请输入 API Key 后再保存。', 'warn');
        return false;
    }
    const ok = await adamiOriginalSaveKeyOnly();
    if(ok !== false){
        apiNotice('API Key 已保存，可以继续拉取视频模型。', 'ok');
        renderProviderOnboarding(provider());
    }
    return ok;
};
...
async function saveVolcengineAssetKeys(){
    ...
    const ak = volcAkInput?.value.trim() || '';
    const sk = volcSkInput?.value.trim() || '';
    if(!ak && !sk){ alert('请输入火山资产库 AK 或 SK'); return; }
    syncEditor();
    const ok = await saveProviders();
    if(ok){
...
async function clearVolcengineAssetKeys(){
    const item = provider();
    if(!item || item.id !== 'volcengine') return;
    if(!confirm('确认清除火山资产库 AK/SK？')) return;
