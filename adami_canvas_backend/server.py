...
from fastapi import FastAPI, HTTPException, WebSocket, WebSocketDisconnect, UploadFile, File, Form, Header, Query, Request
...
GLOBAL_LOOP = None
APP_VERSION = "1.0"
GITHUB_REPO_URL = "https://github.com/tianbao2381357587-del/Adami-Canvas"
GITHUB_VERSION_URL = "https://raw.githubusercontent.com/tianbao2381357587-del/Adami-Canvas/main/VERSION"
GITHUB_TREE_URL = "https://api.github.com/repos/tianbao2381357587-del/Adami-Canvas/git/trees/main?recursive=1"
GITHUB_RAW_ROOT = "https://raw.githubusercontent.com/tianbao2381357587-del/Adami-Canvas/main"
...
def reload_env_globals():
    ...
    CHAT_MODELS = model_list("CHAT_MODELS", "", [])
    VIDEO_MODELS = model_list("VIDEO_MODELS", "", [])
...
def default_api_providers():
    # Adami-Canvas：只保留火山引擎 / 方舟 API 平台
    return [
        {
            "id": "volcengine",
            "name": "火山引擎",
...
def lite_volcengine_only(providers):
    ...
    if not items:
        items = default_api_providers()
    # 只保留一个火山引擎配置；用户保存的模型列表/Key 状态由 env 文件管理
    item = items[0]
    item["id"] = "volcengine"
    item["name"] = item.get("name") or "火山引擎"
...
def merge_default_api_providers(providers):
    return lite_volcengine_only(providers)
...
def load_api_providers():
    ...
    try:
        ...
        return lite_volcengine_only(providers or defaults)
    except Exception as e:
        print(f"加载 API 平台配置失败: {e}")
        return lite_volcengine_only(defaults)

def save_api_providers(providers):
    providers = lite_volcengine_only(providers)
    os.makedirs(DATA_DIR, exist_ok=True)
...
def get_primary_provider_id(providers=None):
    ...
    if non_ms:
        return non_ms["id"]
    return providers[0]["id"] if providers else "volcengine"
...
def fetch_remote_update_notes(url: str, version: str = "", timeout: float = 5.0) -> Dict[str, Any]:
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "Adami-Canvas-Updater"},
            timeout=timeout,
...
# [Adami-Canvas] removed backend route: @app.get("/api/app-info")
...
# [Adami-Canvas] removed backend route: @app.get("/api/update-connectivity/probe")
# [Adami-Canvas] removed backend route: @app.get("/api/update-connectivity")
...
def version_rank(value: str) -> List[int]:
    text = str(value or "").strip()
    parts = version_tuple(text)
    if re.fullmatch(r"\d+\.\d+(?:\.\d+)?", text):
        return [1] + parts
    if re.fullmatch(r"20\d{2}[.\-]\d{1,2}[.\-]\d{1,2}(?:\.\d+)?", text):
        return [0] + parts
    return [1] + parts

def is_legacy_date_version(value: str) -> bool:
    return bool(re.fullmatch(r"20\d{2}[.\-]\d{1,2}[.\-]\d{1,2}(?:\.\d+)?", str(value or "").strip()))

def is_simple_app_version(value: str) -> bool:
    return bool(re.fullmatch(r"\d+\.\d+(?:\.\d+)?", str(value or "").strip()))

def version_gt(a: str, b: str) -> bool:
    ta, tb = version_rank(a), version_rank(b)
    n = max(len(ta), len(tb))
    ta += [0] * (n - len(ta))
    tb += [0] * (n - len(tb))
    return ta > tb
...
def check_update():
    """Check the public Adami-Canvas repository without exposing local data."""
    current = current_app_version()
    github = fetch_remote_version(GITHUB_VERSION_URL, timeout=8.0)
    github["source"] = "github"
    latest: Dict[str, Any] = {}
    update_available = False
    if github.get("ok") and github.get("version"):
        remote_version = github["version"]
        update_available = version_gt(remote_version, current)
        if update_available:
            latest = {"source": "github", "version": remote_version}
            notes = fetch_remote_update_notes(GITHUB_UPDATE_NOTES_URL, remote_version, timeout=5.0)
            latest["update_notes"] = notes if notes.get("ok") else {"version": remote_version, "items": []}
        else:
            github["ignored_legacy_version"] = bool(is_simple_app_version(current) and is_legacy_date_version(remote_version))
            latest = {"source": "github", "version": current, "remote_version": remote_version}
            latest["update_notes"] = read_local_update_notes(current)
    return {
        "current": current,
        "github": github,
        "latest": latest,
        "update_notes": latest.get("update_notes") if latest else {},
...
UPDATE_CODE_PREFIXES = ("static/", "adami_canvas_backend/")

def update_allowed_file(path: str) -> bool:
    path = str(path or "").replace("\\", "/").lstrip("/")
    if not path or any(part in {"", ".", ".."} for part in path.split("/")):
        return False
    # The updater cannot reach personal canvases/assets, API/.env, generated
    # outputs, logs, or bundled runtimes because none are in this allow-list.
    return path in UPDATE_ROOT_FILES or path.startswith(UPDATE_CODE_PREFIXES)
...
def github_json(url: str, use_etag_cache: bool = False):
    headers = {
        ...
        "User-Agent": "Adami-Canvas-Updater",
    }
    cache_key = url
...
def github_bytes(url: str) -> bytes:
    resp = github_get(url, headers={"User-Agent": "Adami-Canvas-Updater"}, timeout=60)
    return resp.content
...
def modelscope_update_file_list() -> List[str]:
    ...
    resp = github_get(MODELSCOPE_TREE_URL, headers={"User-Agent": "Adami-Canvas-Updater"}, timeout=30)
    payload = json.loads(resp.content.decode("utf-8", errors="replace"))
...
def github_update_file_list() -> Tuple[List[str], List[str], List[str]]:
    ...
    if "adami_canvas_backend/server.py" not in root_files:
        raise RuntimeError("GitHub 更新源缺少后端程序文件")
...
def staged_update_file_list(staging_root: str) -> Tuple[List[str], List[str], List[str]]:
    ...
    if "adami_canvas_backend/server.py" not in root_files:
        raise RuntimeError("更新源缺少后端程序文件")
    if not static_files:
...
def update_from_github(req: UpdateRequest = UpdateRequest()):
    """Download application code while preserving all user-owned data."""
    ...
    try:
        ...
        root_files, static_files, _ = stage_update_from_source("github", staging_root)
        with open(os.path.join(staging_root, "VERSION"), "r", encoding="utf-8") as f:
            new_version = (f.read().strip().splitlines() or [""])[0].strip()
        current_version = current_app_version()
        if not version_gt(new_version, current_version):
            raise HTTPException(
                status_code=409,
                detail=f"更新源版本 {new_version or '未知'} 不高于当前版本 {current_version or '未知'}，已取消更新。",
            )

        # Backups stay under data/, a protected directory that the updater can
        # neither download nor replace.
        ...
        if not os.path.isdir(staged_static_dir):
            raise RuntimeError("更新包缺少 static 目录")
        ...
        try:
            ...
            for rel in root_files:
                source_path = os.path.join(staging_root, *rel.split("/"))
                if not os.path.isfile(source_path):
                    raise RuntimeError(f"更新包缺少文件：{rel}")
                target = safe_update_target(rel)
            ...
            for rel in reversed(replaced_root_files):
                ...
                if os.path.isfile(backup_path):
                    ...
                    os.replace(temp_path, target)
            if static_replaced and os.path.isdir(static_dir):
                shutil.rmtree(static_dir, ignore_errors=True)
            if os.path.isdir(backup_static_dir):
                shutil.copytree(backup_static_dir, static_dir)
            raise

        updated = sorted(set(root_files + static_files))
        ...
        return {
            ...
            "count": len(updated),
            "version": new_version,
            "backup_dir": backup_root if os.path.isdir(backup_root) else "",
            "restart_required": True,
            "restart_scheduled": restart_scheduled,
            "preserved": ["data", "assets", "output", "logs", "API/.env", "python", "packages"],
        }
    except HTTPException:
        raise
    except Exception as exc:
        logging.exception("update_from_github failed")
...
def rollback_update(req: RollbackRequest):
    ...
    if not UPDATE_LOCK.acquire(blocking=False):
        raise HTTPException(status_code=409, detail="正在更新中，请稍后再试")
    try:
        backups_root = os.path.abspath(os.path.join(DATA_DIR, "update_backups"))
        backup_dir = os.path.abspath(os.path.join(backups_root, req.name))
        if os.path.commonpath([backups_root, backup_dir]) != backups_root:
        ...
        for dirpath, _, filenames in os.walk(backup_dir):
            for filename in filenames:
                ...
                rel = os.path.relpath(source_path, backup_dir).replace("\\", "/")
                if not update_allowed_file(rel):
                    continue
                target = safe_update_target(rel)
                os.makedirs(os.path.dirname(target), exist_ok=True)
...
ASSET_CATEGORY_MEDIA_TYPES = {"image", "video", "audio"}

def normalize_asset_category_type(value: str) -> str:
    text = str(value or "image").strip().lower()
    if text in {"workflow", "video", "audio"}:
        return text
    return "image"

def ensure_asset_audio_categories(lib: Dict[str, Any]) -> bool:
    changed = False
    required = [
        ("audio", "音频素材", "aud"),
        ("video", "视频素材", "vid"),
    ...
    for library in lib.get("libraries", []) or []:
        if not isinstance(library, dict):
            continue
        cats = library.setdefault("categories", [])
        for cat_type, name, prefix in required:
            has_category = any(
                isinstance(cat, dict) and str(cat.get("type") or "image").lower() == cat_type
                for cat in cats
            )
            if not has_category:
                cats.append({"id": f"{prefix}_{uuid.uuid4().hex[:12]}", "name": name, "type": cat_type, "items": []})
                changed = True
    return changed

def asset_category_accepts_item(cat: Dict[str, Any], item: Dict[str, Any]) -> bool:
    return asset_category_accepts_kind(cat, str(item.get("kind") or "image").lower())

def asset_category_accepts_kind(cat: Dict[str, Any], kind: str) -> bool:
    cat_type = str(cat.get("type") or "image").lower()
...
def list_canvases():
    records = [item for item in iter_canvas_records(include_deleted=False) if normalize_canvas_kind(item.get("kind")) != "smart"]
    return sorted(
        records,
        key=lambda item: (
...
def list_deleted_canvases():
    records = [item for item in iter_canvas_records(include_deleted=True) if normalize_canvas_kind(item.get("kind")) != "smart"]
...
VOLCENGINE_VIDEO_ASPECTS = {"21:9", "16:9", "4:3", "1:1", "3:4", "9:16"}
...
def volcengine_video_ratio(value: str) -> str:
    text = str(value or "16:9").strip()
    return text if text in VOLCENGINE_VIDEO_ASPECTS else "16:9"

def volcengine_video_resolution(value: str) -> str:
    text = str(value or "").strip().lower()
    aliases = {"": "720p", "auto": "720p", "480": "480p", "720": "720p", "1080": "1080p"}
    text = aliases.get(text, text)
    return text if text in VOLCENGINE_VIDEO_RESOLUTIONS else "720p"

def redact_video_test_value(value: Any) -> Any:
    if isinstance(value, dict):
        return {key: redact_video_test_value(val) for key, val in value.items()}
    if isinstance(value, list):
        return [redact_video_test_value(item) for item in value]
    if isinstance(value, str):
        if value.startswith("data:"):
            head = value.split(",", 1)[0]
            mime = head[5:].split(";", 1)[0] or "media"
            return f"<{mime} data URL omitted for preview; {len(value)} chars>"
        if len(value) > 260:
            return value[:180] + f"... <omitted {len(value) - 240} chars> ..." + value[-60:]
    return value
...
def unique_canvas_video_references(payload: CanvasVideoRequest) -> None:
    seen_images = set()
    unique_images = []
    for ref in payload.images or []:
        ...
        if not url or url in seen_images:
            continue
        seen_images.add(url)
        unique_images.append(ref)
    payload.images = unique_images
    ...
    payload.videos = unique_urls(payload.videos)
    payload.audios = unique_urls(payload.audios)

def validate_volcengine_reference_limits(payload: CanvasVideoRequest) -> None:
    unique_canvas_video_references(payload)
    errors = []
    if len(payload.images) > 9:
        errors.append(f"参考图片最多 9 张，当前 {len(payload.images)} 张")
    if len(payload.videos) > 3:
        errors.append(f"参考视频最多 3 个，当前 {len(payload.videos)} 个")
    if len(payload.audios) > 3:
        errors.append(f"参考音频最多 3 个，当前 {len(payload.audios)} 个")

    for label, values in (("视频", payload.videos), ("音频", payload.audios)):
        total = 0.0
        known_count = 0
        for index, url in enumerate(values):
            ...
            if duration < 2 or duration > 15:
                errors.append(f"参考{label} {index + 1} 时长为 {duration:.2f} 秒，单个必须在 2～15 秒之间")
        if known_count and total > 15.05:
            errors.append(f"所有参考{label}总时长最多 15 秒，当前约 {total:.2f} 秒")
    if errors:
...
def local_media_path_for_cloud_upload(ref_url: str, allowed_prefixes=("image/", "video/")) -> str:
    ...
    if ref_url.startswith("http://") or ref_url.startswith("https://"):
        return ""
    if not (ref_url.startswith("/output/") or ref_url.startswith("/assets/")):
        raise HTTPException(status_code=400, detail="临时公网上传只支持画布里的本地图片或视频文件")
    path = output_file_from_url(ref_url)
    if not path:
        raise HTTPException(status_code=404, detail="本地媒体文件不存在或已被删除")
    ct = content_type_for_path(path)
    if not any(ct.startswith(prefix) for prefix in allowed_prefixes):
        raise HTTPException(status_code=400, detail="请选择图片或视频文件再上传到临时公网")
    max_bytes = int(os.getenv("TEMP_SH_MAX_BYTES", str(4 * 1024 * 1024 * 1024)))
    size = os.path.getsize(path)
    if size > max_bytes:
        raise HTTPException(status_code=400, detail=f"媒体文件超过临时公网上传大小限制：{size} bytes")
    return path
...
async def upload_local_video_to_cloud(ref_url: str, service: str = "auto") -> Dict[str, str]:
    ...
    raise HTTPException(status_code=502, detail="临时公网上传失败：" + "；".join(errors))
...
# [Adami-Canvas] removed backend route: @app.post("/api/comfyui/upload-base64")
...
async def import_local_assets_from_urls(payload: LocalAssetUrlImportRequest):
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, headers={"User-Agent": "Adami-Canvas-Asset-Importer/1.0"}) as client:
...
# [Adami-Canvas] removed backend route: @app.post("/api/local-assets/classify")
...
# [Adami-Canvas] removed backend route: @app.get("/api/runninghub/app-info")
# [Adami-Canvas] removed backend route: @app.post("/api/runninghub/submit")
# [Adami-Canvas] removed backend route: @app.post("/api/runninghub/workflow-submit")
# [Adami-Canvas] removed backend route: @app.get("/api/runninghub/workflow-info")
# [Adami-Canvas] removed backend route: @app.get("/api/runninghub/workflows")
# [Adami-Canvas] removed backend route: @app.get("/api/runninghub/workflows/{workflow_id:path}")
# [Adami-Canvas] removed backend route: @app.post("/api/runninghub/workflows/fetch")
# [Adami-Canvas] removed backend route: @app.put("/api/runninghub/workflows/{workflow_id:path}")
# [Adami-Canvas] removed backend route: @app.delete("/api/runninghub/workflows/{workflow_id:path}")
# [Adami-Canvas] removed backend route: @app.get("/api/runninghub/query")
# [Adami-Canvas] removed backend route: @app.post("/api/runninghub/upload-asset")
# [Adami-Canvas] removed backend route: @app.get("/api/jimeng/status")
# [Adami-Canvas] removed backend route: @app.get("/api/jimeng/credit")
# [Adami-Canvas] removed backend route: @app.post("/api/jimeng/logout")
# [Adami-Canvas] removed backend route: @app.post("/api/jimeng/login/start")
# [Adami-Canvas] removed backend route: @app.get("/api/jimeng/login/status")
# [Adami-Canvas] removed backend route: @app.post("/api/jimeng/help")
# [Adami-Canvas] removed backend route: @app.post("/api/jimeng/query-media")
@app.get("/api/config")
async def ai_config():
...
# [Adami-Canvas] removed backend route: @app.get("/api/config/token")
# --- 在线生图 (COMFLY) ---
...
# [Adami-Canvas] removed backend route: @app.post("/api/canvas-comfy-tasks")
# [Adami-Canvas] removed backend route: @app.get("/api/canvas-comfy-tasks/{task_id}")
...
def volcengine_tasks_base_url(provider):
    base_url = video_api_root(provider) or VOLCENGINE_DEFAULT_BASE_URL
    parsed = urllib.parse.urlparse(base_url)
    path = parsed.path.rstrip("/")
    if path.endswith("/api/v3/contents/generations/tasks"):
        return base_url.rstrip("/")
    if path and path != "/api/v3":
        return base_url.rstrip("/")
    if path.endswith("/api/v3"):
        root = base_url[: -len("/api/v3")]
    else:
        root = base_url
    return f"{root.rstrip('/')}/api/v3/contents/generations/tasks"
...
def volcengine_task_status(raw):
    if not isinstance(raw, dict):
        return ""
    ...
    return str(
        ...
        or (data.get("task_status") if isinstance(data, dict) else "")
...
async def build_volcengine_video_test_request(payload: CanvasVideoRequest, provider: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    validate_volcengine_reference_limits(payload)
    provider = provider or get_api_provider(payload.provider_id)
    base_url = video_api_root(provider) or VOLCENGINE_DEFAULT_BASE_URL
    submit_url = video_submit_url_candidates(provider, base_url)[0]
    tasks_base_url = volcengine_tasks_base_url(provider)
    preview_model = str(payload.model or "").strip() or "<请在 API 设置中拉取或手动填写视频模型>"
    body = {
        "model": preview_model,
        "content": [{"type": "text", "text": str(payload.prompt or "").strip()}],
        "duration": volcengine_video_duration(payload.duration),
        "ratio": volcengine_video_ratio(payload.aspect_ratio),
    ...
    def append_image(url: str, role: str):
        if role in {"first_frame", "last_frame"}:
            ...
            return False
        body["content"].append({"type": "image_url", "image_url": {"url": url}, "role": role})
    ...
    for ref in payload.images[:9]:
        ...
        if role in {"first_frame", "last_frame"}:
            append_image(url, role)
        elif payload.multimodal:
            append_image(url, "reference_image")
        elif not frame_roles_used["first_frame"]:
            append_image(url, "first_frame")
    for url in (payload.videos or [])[:3]:
        ...
        if media_url in image_like_urls or looks_like_image_media_url(media_url):
            append_image(media_url, "reference_image" if payload.multimodal else "first_frame")
            continue
        body["content"].extend(await volcengine_video_reference_content_items(media_url))
        volc_video_count += 1
    for url in (payload.audios or [])[:3]:
        duration = probe_local_audio_duration_seconds(url)
        if duration is not None and (duration < 1.8 or duration > 15.2):
            raise HTTPException(status_code=400, detail=f"参考音频时长 {duration:.2f} 秒超出范围：火山方舟 Seedance 参考音频要求在 1.8 ~ 15.2 秒之间。")
        audio_url = volcengine_media_reference_url(url, max_image_size=None)
        if not audio_url:
            continue
        body["content"].append({"type": "audio_url", "audio_url": {"url": audio_url}, "role": volcengine_content_role("", "audio")})
    media_items = [item for item in body["content"] if item.get("type") in {"image_url", "video_url"}]
    audio_items = [item for item in body["content"] if item.get("type") == "audio_url"]
    if audio_items and not media_items:
        raise HTTPException(status_code=400, detail="火山方舟参考音频不能单独作为输入，请同时连接至少一张图片或一个视频。")
    if payload.trusted_asset and body["content"] and body["content"][0].get("type") == "text":
        body["content"][0]["text"] = apply_trusted_asset_prompt_index(body["content"][0].get("text") or "", len(image_like_urls), volc_video_count, 0)
    if payload.seed is not None:
        body["seed"] = payload.seed
    return {
        "target": "volcengine-ark-seedance",
        "method": "POST",
        "url": submit_url,
        "headers": {"Authorization": "Bearer <ARK_API_KEY>", "Content-Type": "application/json"},
        "json": redact_video_test_value(body),
        "task_lifecycle": {
            "create": {
                "method": "POST",
                "url": submit_url,
                "body": "上面的 json。火山返回 id，也就是后续查询用的 task_id。",
            },
            "query": {
                "method": "GET",
                "url": f"{tasks_base_url}/<task_id>",
                "until": "轮询 status，直到 succeeded / failed / expired / cancelled。",
            },
            "list": {
                "method": "GET",
                "url": f"{tasks_base_url}?page_num=1&page_size=20",
                "filters": "可选 filter.status、filter.task_ids、filter.model、filter.service_tier。",
            },
            "cancel_or_delete": {
                "method": "DELETE",
                "url": f"{tasks_base_url}/<task_id>",
                "rule": "queued 可取消；succeeded / failed / expired 可删除；running 不支持删除；cancelled 不可删除。",
            },
            "success_output": "成功后读取 content.video_url。该链接按文档通常只保留 24 小时，建议及时下载保存。",
        },
        "preview_note": "测试预览不会发送到火山方舟，不会消耗 token。媒体 data URL 已折叠显示，真实生成时会使用完整媒体内容。",
    }

@app.post("/api/canvas-video/test")
async def canvas_video_test(payload: CanvasVideoRequest):
    provider = get_api_provider(payload.provider_id)
    if not is_volcengine_provider(provider):
        provider = {"id": "volcengine", "name": "火山方舟", "base_url": VOLCENGINE_DEFAULT_BASE_URL, "protocol": "volcengine", "enabled": True}
    return await build_volcengine_video_test_request(payload, provider)

def get_volcengine_task_provider(provider_id: str = "volcengine") -> Dict[str, Any]:
    provider = get_api_provider(provider_id or "volcengine")
    if not is_volcengine_provider(provider):
        raise HTTPException(status_code=400, detail="当前任务接口只支持火山方舟视频任务。")
    return provider
...
async def canvas_video_tasks(
    ...
    provider = get_volcengine_task_provider(provider_id)
    tasks_url = volcengine_tasks_base_url(provider)
    ...
    for task_id in task_ids or []:
        ...
        if clean_id:
            params.append(("filter.task_ids", clean_id))
    try:
        async with httpx.AsyncClient(timeout=HTTP_TIMEOUT) as client:
            response = await client.get(tasks_url, headers=api_headers(provider=provider), params=params)
            response.raise_for_status()
            raw = parse_json_response(response)
            return {"url": str(response.url), "raw": raw}
    except httpx.HTTPStatusError as exc:
        raise HTTPException(status_code=exc.response.status_code, detail=f"火山方舟任务列表接口错误：{exc.response.text}") from exc
    except httpx.HTTPError as exc:
        log_net_error("火山方舟任务列表网络错误", exc)
        raise HTTPException(status_code=502, detail=f"请求火山方舟任务列表失败：{exc}") from exc

@app.get("/api/canvas-video/tasks/{task_id}")
async def canvas_video_task_detail(task_id: str, provider_id: str = "volcengine"):
    provider = get_volcengine_task_provider(provider_id)
    clean_id = str(task_id or "").strip()
    if not clean_id:
        raise HTTPException(status_code=400, detail="任务 ID 不能为空。")
    task_url = f"{volcengine_tasks_base_url(provider)}/{urllib.parse.quote(clean_id, safe='')}"
    try:
        async with httpx.AsyncClient(timeout=HTTP_TIMEOUT) as client:
            response = await client.get(task_url, headers=api_headers(provider=provider))
            response.raise_for_status()
            raw = parse_json_response(response)
            return {"task_id": clean_id, "status": volcengine_task_status(raw), "videos": video_output_urls(raw), "raw": raw}
    except httpx.HTTPStatusError as exc:
        raise HTTPException(status_code=exc.response.status_code, detail=f"火山方舟任务查询接口错误：{exc.response.text}") from exc
    except httpx.HTTPError as exc:
        log_net_error(f"火山方舟任务查询网络错误 task_id={clean_id}", exc)
        raise HTTPException(status_code=502, detail=f"请求火山方舟任务查询失败：{exc}") from exc

@app.delete("/api/canvas-video/tasks/{task_id}")
async def canvas_video_task_delete(task_id: str, provider_id: str = "volcengine"):
    provider = get_volcengine_task_provider(provider_id)
    clean_id = str(task_id or "").strip()
    if not clean_id:
        raise HTTPException(status_code=400, detail="任务 ID 不能为空。")
    ...
    try:
        async with httpx.AsyncClient(timeout=HTTP_TIMEOUT) as client:
            response = await client.delete(task_url, headers=api_headers(provider=provider))
            response.raise_for_status()
            return {"ok": True, "task_id": clean_id, "raw": parse_json_response(response)}
    except httpx.HTTPStatusError as exc:
        raise HTTPException(status_code=exc.response.status_code, detail=f"火山方舟取消/删除任务接口错误：{exc.response.text}") from exc
    except httpx.HTTPError as exc:
        log_net_error(f"火山方舟取消/删除任务网络错误 task_id={clean_id}", exc)
        raise HTTPException(status_code=502, detail=f"请求火山方舟取消/删除任务失败：{exc}") from exc
...
async def canvas_video(payload: CanvasVideoRequest):
    ...
    requested_model = selected_model(payload.model, "")
        ...
        async with httpx.AsyncClient(timeout=VIDEO_POLL_TIMEOUT) as client:
            ...
            if is_apimart:
                ...
                if is_volcengine and not volc_is_proxy:
                    ...
                    volc_model = selected_model(payload.model, "")
                    body = {
                        "model": volc_model,
                        "content": [
                            {
                    ...
                    body["ratio"] = volcengine_video_ratio(payload.aspect_ratio)
                    resolution = volcengine_video_resolution(payload.resolution)
                    if resolution:
                    ...
                    volc_media_items = [item for item in body["content"] if item.get("type") in {"image_url", "video_url"}]
                    volc_audio_items = [item for item in body["content"] if item.get("type") == "audio_url"]
                    if volc_audio_items and not volc_media_items:
                        raise HTTPException(status_code=400, detail="火山方舟参考音频不能单独作为输入，请同时连接至少一张图片或一个视频。")
                    if payload.trusted_asset and body["content"] and body["content"][0].get("type") == "text":
...
# [Adami-Canvas] removed backend route: @app.post("/api/canvas-llm")
# --- 对话管理 ---

# [Adami-Canvas] removed backend route: @app.get("/api/conversations")
# [Adami-Canvas] removed backend route: @app.post("/api/conversations")
# [Adami-Canvas] removed backend route: @app.get("/api/conversations/{conversation_id}")
# [Adami-Canvas] removed backend route: @app.delete("/api/conversations/{conversation_id}")
# --- 画布管理 ---
...
async def create_canvas(payload: CanvasCreateRequest):
    return {"canvas": new_canvas(payload.title, payload.icon, "classic", payload.project, payload.board_x, payload.board_y)}
...
# [Adami-Canvas] removed backend route: @app.get("/api/smart-canvas/prompt-templates")
...
# [Adami-Canvas] removed backend route: @app.post("/api/canvas-workflows/export")
# [Adami-Canvas] removed backend route: @app.post("/api/canvas-workflows/export-to-library")
# [Adami-Canvas] removed backend route: @app.post("/api/asset-library/workflows/upload")
# [Adami-Canvas] removed backend route: @app.post("/api/canvas-workflows/import")
...
# [Adami-Canvas] removed backend route: @app.post("/api/smart-canvas/group-export")
@app.get("/api/asset-library")
async def get_asset_library():
    lib = load_asset_library()
    if ensure_asset_audio_categories(lib):
        save_asset_library(lib)
    return {"library": lib}
...
async def create_asset_library(payload: AssetLibraryRequest):
    ...
    library["categories"].append({"id": f"aud_{uuid.uuid4().hex[:12]}", "name": "音频素材", "type": "audio", "items": []})
    library["categories"].append({"id": f"vid_{uuid.uuid4().hex[:12]}", "name": "视频素材", "type": "video", "items": []})
...
async def create_asset_library_category(payload: AssetLibraryCategoryRequest):
    ...
    if not library:
        raise HTTPException(status_code=404, detail="资产库不存在")
    cat_type = normalize_asset_category_type(payload.type)
...
async def add_asset_library_item(payload: AssetLibraryAddRequest):
    ...
    if not cat:
        raise HTTPException(status_code=404, detail="分类不存在")
    if str(cat.get("type") or "image").lower() not in ASSET_CATEGORY_MEDIA_TYPES:
    ...
    if not asset_category_accepts_kind(cat, asset_library_media_kind(src)):
        raise HTTPException(status_code=400, detail="当前分组只能保存对应类型的素材")
    _, item = make_asset_library_item(src, payload.name or os.path.basename(src), subdir=cat.get("dir") or "")
    if not asset_category_accepts_item(cat, item):
        raise HTTPException(status_code=400, detail="当前分组只能保存对应类型的素材")
...
async def import_shared_folder_files(payload: SharedFolderImport):
    ...
    for rel in (payload.paths or [])[:200]:
        ...
        if ext not in SHARED_MEDIA_EXTS:
            continue
        if not asset_category_accepts_kind(cat, asset_library_media_kind(abs_path)):
            continue
...
# [Adami-Canvas] removed backend route: @app.post("/api/asset-library/items/classify")
...
# [Adami-Canvas] removed backend route: @app.post("/api/chat")
# [Adami-Canvas] removed backend route: @app.post("/api/chat/agent")
# [Adami-Canvas] removed backend route: @app.post("/api/chat/stream")
# --- 历史记录 ---

# [Adami-Canvas] removed backend route: @app.get("/api/history")
# [Adami-Canvas] removed backend route: @app.get("/api/queue_status")
# [Adami-Canvas] removed backend route: @app.post("/api/history/delete")
# --- ModelScope 角度控制 ---

# [Adami-Canvas] removed backend route: @app.post("/api/angle/poll_status")
# [Adami-Canvas] removed backend route: @app.post("/api/angle/generate")
# --- ModelScope Z-Image 云端生图 ---

# [Adami-Canvas] removed backend route: @app.post("/generate")
# --- ModelScope 通用图片生成（支持图生图） ---

# [Adami-Canvas] removed backend route: @app.post("/api/ms/generate")
# --- 本地 ComfyUI 生图 ---

# [Adami-Canvas] removed backend route: @app.post("/api/generate")
# --- ComfyUI 工作流管理 ---
...
# [Adami-Canvas] removed backend route: @app.get("/api/comfyui/instances")
# [Adami-Canvas] removed backend route: @app.put("/api/comfyui/instances")
# [Adami-Canvas] removed backend route: @app.get("/api/workflows")
# [Adami-Canvas] removed backend route: @app.get("/api/workflows/{name:path}")
# [Adami-Canvas] removed backend route: @app.post("/api/workflows")
# [Adami-Canvas] removed backend route: @app.put("/api/workflows/{name:path}/config")
# [Adami-Canvas] removed backend route: @app.delete("/api/workflows/{name:path}")
# [Adami-Canvas] removed backend route: @app.post("/api/workflows/{name:path}/run")
